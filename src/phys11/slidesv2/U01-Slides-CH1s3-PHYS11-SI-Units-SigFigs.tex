...
\note{
    ...
    \begin{itemize}
        \item Read through objectives, emphasize these are testable skills
        \item Ask: "Who has used scientific notation before?"
        \item Preview: We'll practice all three of these today
    \end{itemize}
}
...
\end{frame}
\note{
    \textbf{Key point:} Sig figs = precision, NOT accuracy!

    \begin{itemize}
        \item Point to each ruler - ask students to read measurement
        \item Common confusion: students think more digits = better
        \item Emphasize: we can only report what we can measure
    \end{itemize}

    \textbf{Check:} "Why can't we just write down more decimal places?"
}
...
\end{frame}
\note{
    \textbf{Wrap-up reminders:}
    \begin{itemize}
        \item Homework: SNAP p.2-17
        \item Quiz next class on sig fig rules
        \item Office hours: lunch in room 204
    \end{itemize}

    Ask: "What's one thing you learned today?"
}
