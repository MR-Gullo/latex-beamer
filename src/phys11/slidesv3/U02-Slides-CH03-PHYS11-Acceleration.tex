...
\end{alertblock}
\note{[P0][Fig 3.1: Airplane landing with acceleration opposite to velocity]  "Plane landing at St. Maarten - velocity decreasing"\\\\
...
[TEACHING HINT] Ask: 'What direction do you feel pushed in your seat when braking? That's the direction of acceleration!'}
\end{frame}
...
\end{itemize}
\note{[P0] [Fig 3.2: Two cars showing speeding up and slowing down with vectors] "Two scenarios with vector arrows"\\\\
...
[TEACHING HINT] Have students draw velocity and acceleration vectors for walking to a stop - reinforces that braking is acceleration}
\end{frame}
...
\pause
\textbf{Step 2:} Calculate $\Delta \vel{v} = \vel{v_f} - \vel{v_0} = 8.333 - 0 = 8.333$ m/s

\pause
\textbf{Step 3:} Apply the equation
$$\bar{\accel{a}} = \frac{\Delta \vel{v}}{\Delta \tvar{t}} = \frac{8.333\text{ m/s}}{20.0\text{ s}} = \boxed{+0.417 \text{ m/s}^2}$$
...
\textbf{Step 1:} Calculate $\Delta \vel{v} = \vel{v_f} - \vel{v_0} = 0 - 8.333 = -8.333$ m/s

\pause
\textbf{Step 2:} Apply the equation
$$\bar{\accel{a}} = \frac{\Delta \vel{v}}{\Delta \tvar{t}} = \frac{-8.333\text{ m/s}}{8.00\text{ s}} = \boxed{-1.04 \text{ m/s}^2}$$
...
\begin{align}
\disp{d} &= \disp{d_0} + \bar{\vel{v}}\tvar{t} \\
\bar{\vel{v}} &= \frac{\vel{v_0} + \vel{v_f}}{2} \\
\vel{v} &= \vel{v_0} + \accel{a}\tvar{t} \\
\disp{d} &= \disp{d_0} + \vel{v_0} \tvar{t} + \frac{1}{2}\accel{a}\tvar{t}^2 \\
...
\pause
\textbf{Key insight:}
$$\bar{\vel{v}} = \frac{\disp{d}}{\tvar{t}} \quad \text{(when starting from origin)}$$

\pause
The slope IS the velocity!
\note{[P0] [Fig 3.6: Displacement vs time linear graph] "Graph of displacement vs time - directly proportional relationship"\\\\
...
[TEACHING HINT] Trace finger along line: 'At any point, slope tells instantaneous velocity. Constant slope means constant velocity!'}
\end{frame}
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys11-acceleration-fig3-7.jpg}
\caption{Slope of velocity vs time gives acceleration}
\end{figure}

\pause
\textbf{Key insight:}
$$\accel{a} = \frac{\vel{v}}{\tvar{t}} \quad \text{(when starting from rest)}$$

\pause
The slope IS the acceleration!
\note{[P0] [Fig 3.7: Line graph showing time vs velocity] "Graph of velocity vs time - shows directly proportional relationship"\\\\
[P1] "Slope equals velocity change over time equals acceleration"\\\\
[P2] [THE REVELATION] "Every v-vs-t graph encodes acceleration - this linear graph shows constant acceleration"\\\\
...
[TEACHING HINT] Point to the graph: 'What would zero acceleration look like? A horizontal line!'}
\end{frame}
...
\begin{itemize}
\item \tvar{Time} not given? Use equation 5: $\vel{v}^2 = \vel{v_0}^2 + 2\accel{a}(\disp{d} - \disp{d_0})$
\item \disp{Displacement} not needed? Use equation 3: $\vel{v} = \vel{v_0} + \accel{a}\tvar{t}$
\item From rest ($\vel{v_0} = 0$)? Equations simplify!
\end{itemize}
\end{block}
...
\textbf{Step 1:} Choose equation 5 (no \tvar{time}): $\vel{v}^2 = \vel{v_0}^2 + 2\accel{a}(\disp{d} - \disp{d_0})$

\pause
\textbf{Step 2:} Since $\vel{v_0} = 0$: $\vel{v}^2 = 2\accel{a}(\disp{d} - \disp{d_0})$

\pause
\textbf{Step 3:} Substitute values
$$\vel{v}^2 = 2(26.0)(402) = 2.09 \times 10^4 \text{ m}^2/\text{s}^2$$

\pause
\textbf{Step 4:} Take square root
$$\vel{v} = \sqrt{2.09 \times 10^4} = \boxed{145 \text{ m/s}}$$
...
\begin{center}
\Large $\boxed{\grav{g} = 9.80 \text{ m/s}^2}$
\end{center}
Near Earth's surface, all objects fall with this \accel{acceleration} (ignoring air resistance).
\end{block}

\pause
\vspace{0.3cm}

\textbf{Convention:} When using $\grav{g}$ in equations, give it a negative sign because gravity points downward.
...
\textbf{Position:} $\disp{y} = \disp{y_0} + \vel{v_0} \tvar{t} + \frac{1}{2}\accel{a}\tvar{t}^2$

\pause
$$\disp{y} = 0 + (13.0)(1.00) + \frac{1}{2}(-9.80)(1.00)^2 = \boxed{8.10 \text{ m}}$$

\pause
\textbf{Velocity:} $\vel{v} = \vel{v_0} + \accel{a}\tvar{t}$

\pause
$$\vel{v} = 13.0 + (-9.80)(1.00) = \boxed{3.20 \text{ m/s}}$$
...
\begin{enumerate}
\item \accel{Acceleration} = rate of change of \vel{velocity} (direction matters!) \pause
\item $\bar{\accel{a}} = \Delta \vel{v} / \Delta \tvar{t}$ - the definition of \accel{acceleration} \pause
\item Five kinematic equations predict motion \pause
\item Graphs reveal \accel{acceleration} as slopes \pause
\item $\grav{g} = 9.80$ m/s$^2$ - Earth's gravitational \accel{acceleration} \pause
...
\begin{align}
\bar{\accel{a}} &= \frac{\Delta \vel{v}}{\Delta \tvar{t}} = \frac{\vel{v_f} - \vel{v_0}}{\tvar{t_f} - \tvar{t_0}} \\
\disp{d} &= \disp{d_0} + \bar{\vel{v}}\tvar{t} \\
\bar{\vel{v}} &= \frac{\vel{v_0} + \vel{v_f}}{2} \\
\vel{v} &= \vel{v_0} + \accel{a}\tvar{t} \\
\disp{d} &= \disp{d_0} + \vel{v_0} \tvar{t} + \frac{1}{2}\accel{a}\tvar{t}^2 \\
\vel{v}^2 &= \vel{v_0}^2 + 2\accel{a}(\disp{d} - \disp{d_0}) \\
\grav{g} &= 9.80 \text{ m/s}^2
\end{align}
