...
\end{exampleblock}
\note{[P0][Fig 4.1: Dolphin jumping in curved path] "The dolphin jumping out of water - elegant, beautiful. Teaching hint: This opening image hooks students by showing real-world motion that follows mathematical laws - the parabolic trajectory foreshadows projectile motion while introducing the idea that nature is predictable."\\\\
...
\end{exampleblock}
\note{[P0][Fig 4.10: Tarzan hanging motionless] "Free-body diagram: our most important tool. Teaching hint: Fig 4.10 shows Tarzan in equilibrium - use this to introduce how we abstract complex systems (person on vine) into simple diagrams (T up, W down). The equal-length vectors visually reinforce net force = 0."\\\\
...
\note{[P0][Fig 4.10: Tarzan hanging motionless] "Object hanging from rope - two forces. Teaching hint: Reuse Fig 4.10 here to show consistency in diagrams - students see the same image with different pedagogical focus (first introducing FBDs, now analyzing equilibrium). This repetition builds pattern recognition."\\\\
...
\note{[P0][Fig 4.3: Box on surface with friction force] "Box sliding across floor. Teaching hint: Fig 4.3 explicitly shows friction vector pointing opposite to velocity vector - this visual reinforces that friction opposes motion. The inclusion of normal force (N up) and weight (mg down) in the FBD sets up future discussion of friction depending on N."\\\\
...
\begin{center}
\Large $\boxed{\vec{\force{F}}_{\text{net}} = \mass{m}\vec{\accel{a}}}$
\end{center}
Net \force{force} equals \mass{mass} times \accel{acceleration}.
\end{block}
...
$$\vec{\force{F}}_{\text{net}} = \mass{m}\vec{\accel{a}}$$

\pause
\vspace{0.3cm}

\textbf{\accel{Acceleration} is directly proportional to \force{force}:}

Double the \force{force} $\rightarrow$ double the \accel{acceleration}

\pause
\vspace{0.3cm}

\textbf{\accel{Acceleration} is inversely proportional to \mass{mass}:}

Double the \mass{mass} $\rightarrow$ half the \accel{acceleration}

\pause
\vspace{0.3cm}

\begin{alertblock}{Key Insight}
Same \force{force} on different \mass{masses} produces different \accel{accelerations}!
\end{alertblock}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth,height=0.55\textheight,keepaspectratio]{phys11-dynamics-fig4-5.jpg}
\end{figure}

\pause
Same \force{force}, different \mass{masses}, different \accel{accelerations}.
\note{[P0][Fig 4.5: Person pushing basketball vs car] "Person pushing basketball vs pushing car - same force F. Teaching hint: Fig 4.5 shows three diagrams (a: basketball with mass m1, b: car with mass m2, c: identical FBDs) - perfect visual for showing F=ma. Same force F, different masses produce different accelerations (a1 >> a2). The identical FBDs in diagram c reinforce that the equation is what determines the outcome."\\\\
...
A 1-kg \mass{mass} accelerated at 1 m/s² requires 1 N of \force{force}.
...
\textbf{\force{Weight}} is the gravitational \force{force} on an object.
...
\pause
For a falling object: $\vec{\force{F}}_{\text{net}} = \vec{\force{W}}$ and $\vec{\accel{a}} = \vec{\grav{g}}$
...
\begin{block}{The Universal Law}
\begin{center}
\Large $\boxed{\force{W} = \mass{m}\grav{g}}$
\end{center}
\force{Weight} equals \mass{mass} times gravitational \accel{acceleration}.
\end{block}
...
On Earth: $\grav{g} = 9.8 \text{ m/s}^2$

\pause
On the Moon: $\grav{g} = 1.67 \text{ m/s}^2$

\pause
\vspace{0.3cm}

\begin{exampleblock}{Example: 1.0-kg \mass{mass}}
\begin{itemize}
\item Earth: $\force{W} = (1.0)(9.8) = 9.8$ N \pause
\item Moon: $\force{W} = (1.0)(1.67) = 1.7$ N
\end{itemize}
\end{exampleblock}
...
\textbf{Same \mass{mass}, different \force{weight}!}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth,height=0.35\textheight,keepaspectratio]{phys11-dynamics-fig4-6.jpg}
\end{figure}
\end{column}
\begin{column}{0.5\textwidth}
\begin{exampleblock}{The Challenge}
Net external force: 51 N\\
Mass: 24 kg
...
\textit{3 min, silent - Can you predict its acceleration?}
\note{[THE CHALLENGE] Can they decode motion like Newton? Teaching hint: Fig 4.6 shows man pushing lawn mower with F-net vector - provides visual context for the problem, helping students see this is a real-world application of F=ma.\\\\
...
\textbf{E - Equation:} $\force{F}_{\text{net}} = \mass{m}\accel{a}$

\pause
\vspace{0.2cm}

\textbf{Rearrange:} $\accel{a} = \frac{\force{F}_{\text{net}}}{\mass{m}}$

\pause
\vspace{0.2cm}

\textbf{S - Substitute:} $\accel{a} = \frac{51 \text{ N}}{24 \text{ kg}}$

\pause
\vspace{0.2cm}

$$\boxed{\accel{a} = 2.1 \text{ m/s}^2}$$
...
\begin{center}
\Large $\boxed{\vec{\force{F}}_{A \to B} = -\vec{\force{F}}_{B \to A}}$
\end{center}
When object A exerts a \force{force} on object B,\\
object B exerts an equal and opposite \force{force} on object A.
\end{block}
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys11-dynamics-fig4-8.jpg}
\end{figure}
...
\note{[P0][Fig 4.8: Swimmer pushing off wall underwater] "Swimmer pushes off pool wall. Teaching hint: Fig 4.8 shows swimmer with feet on wall, force vectors showing wall-on-feet force opposing swimmer's push - vivid action-reaction example. Includes buoyancy and weight forces in FBD, showing third law pairs exist even in multi-force systems."\\\\
...
\textbf{Physicist:} "Same \force{force}. Different \accel{acceleration}."
\end{alertblock}

\pause
\vspace{0.3cm}

$\force{F} = \mass{m}\accel{a}$

\pause
\vspace{0.3cm}

Bug has tiny \mass{mass} $\rightarrow$ huge \accel{acceleration} $\rightarrow$ splat

\pause
\vspace{0.3cm}

Truck has huge \mass{mass} $\rightarrow$ tiny \accel{acceleration} $\rightarrow$ barely notices
...
\textbf{\force{Weight}} pulls down

\pause
\vspace{0.3cm}

\textbf{\normal{Normal force}} pushes up (perpendicular to surface)

\pause
\vspace{0.3cm}

\begin{block}{For horizontal surface}
$$\normal{N} = \mass{m}\grav{g}$$
\end{block}

\pause
\vspace{0.3cm}

Equal magnitude, opposite direction $\rightarrow$ net \force{force} = 0
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth,height=0.35\textheight,keepaspectratio]{phys11-dynamics-fig4-9.jpg}
\end{figure}

\pause
\textbf{\force{Tension}} is the pulling \force{force} along a connector (rope, string, cable).

\pause
\vspace{0.3cm}

For a stationary \mass{mass}: $\force{T} = \force{W} = \mass{m}\grav{g}$

\pause
\vspace{0.3cm}

Rope pulls up on mass, mass pulls down on rope.
\note{[P0][Fig 4.9: Hand pulling rope with mass attached] "Person holding mass on rope. Teaching hint: Fig 4.9 shows tension force vectors at both ends of rope - crucial for showing that tension is transmitted through the rope. The caption emphasizes 'rope is the medium that transmits forces of equal magnitude but opposite directions' - perfect for introducing tension as action-reaction pair."\\\\
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.4\textheight,keepaspectratio]{phys11-dynamics-fig4-7.jpg}
\end{figure}
...
This forward \force{force} is called \textbf{\force{thrust}}.
...
\end{alertblock}
\note{[P0][Fig 4.7: Rocket sled with thrust forces] "Rockets move by expelling gas backward. Teaching hint: Fig 4.7 shows rocket sled with four thrust force vectors pointing right (forward), acceleration vector also right, friction opposing left - perfect for showing net force = thrust - friction. The FBD includes all forces (thrust, friction, normal, weight), reinforcing systematic force analysis."\\\\
...
\begin{figure}
\centering
\includegraphics[width=\textwidth,height=0.35\textheight,keepaspectratio]{phys11-dynamics-fig4-11.jpg}
\end{figure}
\end{column}
\begin{column}{0.55\textwidth}
\begin{exampleblock}{The Challenge}
Foot applies 150 N backward\\
Friction: 24.0 N\\
Mass: 84.0 kg total
...
\textit{3 min, silent - Can you decode this system?}
\note{[THE CHALLENGE] Can they identify the system and find net force? Teaching hint: Fig 4.11 shows teacher pushing cart with system boundary drawn around teacher+cart - critical for showing which forces are external (floor force forward, friction backward) vs internal (teacher's push on cart). FBD shows only external forces acting on the system.\\\\
[SAY] "Try this on your own. It's okay to get stuck."\\\\
[TIMING] 3-4 min SILENT individual work\\\\
...
\textbf{Step 1:} Find net \force{force}
$$\force{F}_{\text{net}} = \force{F}_{\text{floor}} - \force{f} = 150 - 24.0 = 126 \text{ N}$$

\pause
\vspace{0.2cm}

\textbf{Step 2:} Find total \mass{mass}
$$\mass{m} = 65.0 + 12.0 + 7.0 = 84.0 \text{ kg}$$

\pause
\vspace{0.2cm}

\textbf{Step 3:} Calculate \accel{acceleration}
$$\accel{a} = \frac{\force{F}_{\text{net}}}{\mass{m}} = \frac{126}{84.0} = \boxed{1.5 \text{ m/s}^2}$$
...
\item \textbf{Law of Acceleration:} $\vec{\force{F}}_{\text{net}} = \mass{m}\vec{\accel{a}}$ connects \force{force}, \mass{mass}, and motion \pause
...
\begin{align}
\text{Newton's Second Law:} \quad \vec{\force{F}}_{\text{net}} &= \mass{m}\vec{\accel{a}} \\
\text{Weight:} \quad \force{W} &= \mass{m}\grav{g} \\
\text{Friction:} \quad \force{f} &= \fric{\mu} \normal{N} \\
\text{Normal Force (horizontal):} \quad \normal{N} &= \mass{m}\grav{g}
\end{align}
