...
\note{[P0] "Two types of reflection"\\\\
...
\end{block}
\note{[P0] "How flat mirrors create images"\\\\
...
\textbf{Focal length (f):} Distance from mirror to focal point
...
\pause
Concave: rays \textbf{converge} (f positive). Convex: rays \textbf{diverge} (f negative).
\note{[P0] "Focal points for curved mirrors"\\\\
...
\note{[P0] "Ray diagrams show image formation"\\\\
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys11-geometric-optics-fig16-12.jpg}
\end{figure}
...
\note{[P0] "Parabolic mirrors have special property"\\\\
[P1] "Light source at focal point creates parallel rays - perfect for headlights"\\\\
[P2] "Reverse works too: parallel rays from Sun focus to one point, generate heat"\\\\
[THE WONDER] Same geometry explains car headlights and solar power plants}
\end{frame}
...
\note{[P0] "Convex mirrors in stores"\\\\
...
\begin{itemize}
\item Negative $d_i$ $\rightarrow$ virtual image \pause
\item Negative $h_i$ $\rightarrow$ inverted image \pause
\item Concave: $f$ positive; Convex: $f$ negative
\end{itemize}
\end{alertblock}
...
\begin{itemize}
\item $d_o = 6.0$ m
\item $d_i = -1.0$ m (virtual)
\end{itemize}

\textbf{Find:} Focal length $f$
...
\item What sign did you use for $d_i$? Why negative?
...
$$f = \frac{d_i d_o}{d_o + d_i}$$

\pause
\vspace{0.2cm}

\textbf{Substitute:} $d_i = -1.0$ m, $d_o = 6.0$ m

\pause
\vspace{0.2cm}

$$f = \frac{(-1.0)(6.0)}{6.0 + (-1.0)} = \frac{-6.0}{5.0}$$
...
\pause
\textbf{Check:} Negative $f$ confirms convex mirror!
...
\textbf{Where:}
\begin{itemize}
\item $n$ = index of refraction (dimensionless) \pause
\item $c = 3.00 \times 10^8$ m/s (speed of light in vacuum) \pause
\item $v$ = speed of light in the material
\end{itemize}

\pause
Because $c > v$ always, $n \geq 1$ always.
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth,height=0.4\textheight,keepaspectratio]{phys11-geometric-optics-fig16-16.jpg}
\end{figure}

\pause
\begin{block}{Universal Law: The Bending Rule}
\begin{center}
\Large $\boxed{n_1 \sin\theta_1 = n_2 \sin\theta_2}$
\end{center}
This predicts exactly how light bends at any boundary.
\end{block}
\note{[P0] "Light bending at boundary between media"\\\\
[P1] [THE REVELATION] "Snell's law: n-1 sin-theta-1 equals n-2 sin-theta-2"\\\\
[ALGEBRA] "n-one sine-theta-one equals n-two sine-theta-two"\\\\
- Theta measured from normal to surface\\\\
- Greater n means slower light, more bending\\\\
[THE WONDER] Works for any two materials - air-water, glass-diamond, anything}
\end{frame}
...
\pause
\textbf{Dispersion:} Index of refraction varies slightly with wavelength

\pause
White light separates into colors: red bends least, violet bends most
\note{[P0] "Prism separating white light into rainbow"\\\\
...
\pause
When $\theta_1 > \theta_c$, \textbf{all} light reflects back - no refraction!
\note{[P0] "Total internal reflection: 100 percent reflection"\\\\
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth,height=0.45\textheight,keepaspectratio]{phys11-geometric-optics-fig16-22.jpg}
\end{figure}

\pause
Diamond critical angle: only $24.4°$!

\pause
Light enters easily but struggles to exit $\rightarrow$ sparkle!
\note{[P0] "Why diamonds sparkle"\\\\
[P1] "Diamond has very high index of refraction, critical angle only 24.4 degrees"\\\\
[P2] "Light enters easily but trapped inside, exits only at specific angles"\\\\
- Facets designed to trap light\\\\
- Concentrated exits create sparkle\\\\
[THE WONDER] Geometry explains beauty}
\end{frame}
...
\end{itemize}
\note{[P0] "Fiber optics use total internal reflection"\\\\
...
\begin{exampleblock}{The Challenge (3 min, silent)}
Light enters glass ($n = 1.50$) from air ($n = 1.00$) at $45.0°$ to the normal.
...
\textbf{Find:} Angle of refraction $\theta_2$
...
\item How did you rearrange for $\theta_2$?
...
\textbf{Snell's Law:} $n_1 \sin\theta_1 = n_2 \sin\theta_2$

\pause
\textbf{Rearrange:} $\sin\theta_2 = \frac{n_1 \sin\theta_1}{n_2}$

\pause
\textbf{Substitute:} $\sin\theta_2 = \frac{(1.00)(\sin 45.0°)}{1.50} = \frac{0.707}{1.50}$

\pause
$$\sin\theta_2 = 0.471$$

\pause
$$\boxed{\theta_2 = \sin^{-1}(0.471) = 28.1°}$$
...
\pause
\textbf{Convex (converging):} Parallel rays converge to focal point ($f$ positive)

\pause
\textbf{Concave (diverging):} Parallel rays diverge from focal point ($f$ negative)
\note{[P0] "Two types of lenses"\\\\
...
\textbf{Where:}
\begin{itemize}
\item $P$ = power in diopters (D) or m$^{-1}$ \pause
\item $f$ = focal length in meters
\end{itemize}

\pause
\vspace{0.3cm}

Shorter focal length = stronger lens = higher power
...
\pause
Object beyond $f$: Real, inverted image on far side

\pause
Object inside $f$: Virtual, upright, magnified image on near side
\note{[P0] "Ray diagram for converging lens"\\\\
...
\begin{itemize}
\item Converging lens: $f$ positive; Diverging lens: $f$ negative
\item Negative $d_i$ $\rightarrow$ virtual image
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys11-geometric-optics-fig16-34.jpg}
\end{figure}
...
\pause
\textbf{Ciliary muscles:} Change lens shape to adjust focal length

\pause
Eye adjusts power to keep image distance constant for all object distances!
\note{[P0] "The eye as optical instrument"\\\\
[P1] "Cornea and lens act as single converging lens, form real image on retina"\\\\
[P2] "Ciliary muscles change lens shape to adjust focal length"\\\\
[P3] "Eye adjusts power to focus objects at different distances - accommodation"\\\\
- Retina is fixed distance from lens\\\\
- Most refraction occurs at cornea, not lens}
\end{frame}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth,height=0.6\textheight,keepaspectratio]{phys11-geometric-optics-fig16-28.jpg}
\end{figure}
...
\note{[P0] "Two common vision defects"\\\\
[P1] "Nearsighted: eye overconverges rays, image forms in front of retina"\\\\
[P2] "Farsighted: eye underconverges rays, image forms behind retina"\\\\
- Nearsighted: see close clearly, distant blurry\\\\
- Farsighted: see distant clearly, close blurry}
\end{frame}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth,height=0.6\textheight,keepaspectratio]{phys11-geometric-optics-fig16-31.jpg}
\end{figure}
...
\note{[P0] "Correcting vision with eyeglasses"\\\\
[P1] "Nearsighted: diverging lens reduces overall power, moves image back to retina"\\\\
[P2] "Farsighted: converging lens increases overall power, moves image forward to retina"\\\\
[THE WONDER] Physics corrects biology - lenses compensate for eye shape}
\end{frame}
...
\end{itemize}
\note{[P0] "Compound microscope uses two lenses"\\\\
...
\note{[P0] "Telescope designs using lenses"\\\\
...
\textbf{Given:}
\begin{itemize}
\item $f = 8.00$ cm $= 0.0800$ m
\end{itemize}

\textbf{Find:} Power $P$ in diopters
...
\begin{enumerate}
\item Which equation relates power and focal length?
\item What units did you use for focal length?
...
\textbf{Given:} $f = 8.00$ cm

\pause
\textbf{Convert to meters:} $f = 0.0800$ m

\pause
\vspace{0.2cm}

\textbf{Power equation:} $P = \frac{1}{f}$

\pause
\vspace{0.2cm}

$$P = \frac{1}{0.0800 \text{ m}}$$
...
\begin{enumerate}
\item Law of Reflection: $\theta_r = \theta_i$ (mirrors create virtual images) \pause
\item Curved mirrors use focal points to form real or virtual images \pause
\item Snell's Law: $n_1\sin\theta_1 = n_2\sin\theta_2$ (light bends at boundaries) \pause
...
\item Your eye is a variable-power converging lens \pause
...
\begin{align}
\text{Law of Reflection:} \quad &\theta_r = \theta_i \\
\text{Index of refraction:} \quad &n = \frac{c}{v} \\
\text{Snell's Law:} \quad &n_1\sin\theta_1 = n_2\sin\theta_2 \\
\text{Critical angle:} \quad &\theta_c = \sin^{-1}\left(\frac{n_2}{n_1}\right) \\
\text{Lens/Mirror:} \quad &\frac{1}{f} = \frac{1}{d_i} + \frac{1}{d_o} \\
\text{Magnification:} \quad &m = \frac{h_i}{h_o} = -\frac{d_i}{d_o} \\
\text{Lens power:} \quad &P = \frac{1}{f}
\end{align}
