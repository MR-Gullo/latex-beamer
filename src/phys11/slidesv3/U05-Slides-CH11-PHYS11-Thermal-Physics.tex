...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth,height=0.6\textheight,keepaspectratio]{phys11-thermal-fig11-1.jpg}
\end{figure}

\pause
How does thermal energy travel from the arc to your skin meters away?
\note{[Fig 11.1: Sparks fly from welder's mask] "Intro hook - radiation transfers heat without contact"\\\\
[P0] "Welder's arc - hot enough to melt metal"\\\\
...
\begin{center}
\Large $\kenergy{KE} = \frac{1}{2}\mass{m}\vel{v}^2$
\end{center}
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys11-thermal-fig11-2.jpg}
...
\end{itemize}
\note{[Fig 11.2: F/C/K scales showing 0-373K with freeze/boil points] "Visual anchor for scale comparison - students see absolute zero, body temp, phase transitions aligned"\\\\
[P0] "Three temperature scales - all measure same thing"\\\\
...
\begin{align*}
\temp{T_{\circ F}} &= \frac{9}{5}\temp{T_{\circ C}} + 32 \\
\temp{T_{\circ C}} &= \frac{5}{9}(\temp{T_{\circ F}} - 32) \\
\temp{T_{K}} &= \temp{T_{\circ C}} + 273.15 \\
\temp{T_{\circ C}} &= \temp{T_{K}} - 273.15
\end{align*}
\end{block}
...
\textbf{Step 1:} Convert F to C using $\temp{T_{\circ C}} = \frac{5}{9}(\temp{T_{\circ F}} - 32)$

\pause
\vspace{0.2cm}

\textbf{Substitute:} $\temp{T_{\circ C}} = \frac{5}{9}(98.6 - 32) = \frac{5}{9}(66.6)$

\pause
\vspace{0.2cm}

$$\temp{T_{\circ C}} = \boxed{37.0\text{ C}}$$

\pause
\vspace{0.2cm}

\textbf{Step 2:} Convert C to K using $\temp{T_{K}} = \temp{T_{\circ C}} + 273.15$

\pause
\vspace{0.2cm}

$$\temp{T_{K}} = 37.0 + 273.15 = \boxed{310.2\text{ K}}$$
...
\begin{center}
\Large $\boxed{\heat{Q} = \mass{m}\specheat{c}\Delta \temp{T}}$
\end{center}
\heat{Heat} transferred = \mass{mass} × \specheat{specific heat} × \temp{temperature} change
\end{block}
...
\begin{itemize}
\item $\heat{Q}$ = \heat{heat} transferred (J)
\item $\mass{m}$ = \mass{mass} (kg)
\item $\specheat{c}$ = \specheat{specific heat} (J/kg·C)
\item $\Delta \temp{T}$ = \temp{temperature} change (C or K)
\end{itemize}
...
\end{itemize}
\note{[Fig 11.3: Fireplace with arrows - radiation rays, convection loop, conduction] "Perfect synthesis image - use to have students identify which mode does what before revealing"\\\\
[P0] "Fireplace demonstrates all three modes simultaneously"\\\\
...
\note{[Fig 11.4: Particle collision - fast particle hits slow one, velocities equalize] "Microscopic mechanism visible - pause here and ask students to predict what happens to velocities after collision"\\\\
[P0] "Conduction: direct physical contact"\\\\
...
\note{[Fig 11.5: Furnace convection loop in house] "Macro-scale - relatable to student homes"\\\\
[Fig 11.6: Pot boiling with circulation arrows] "Micro-scale - observable in lab/kitchen - both show same principle at different scales"\\\\
[P0] "Convection: heat transfer by moving fluid"\\\\
...
\textbf{Step 1:} Calculate \temp{temperature} change
$$\Delta \temp{T} = \temp{T_f} - \temp{T_i} = 80 - 20 = 60\text{ C}$$

\pause
\vspace{0.2cm}

\textbf{Step 2:} Use $\heat{Q} = \mass{m}\specheat{c}\Delta \temp{T}$

\pause
\vspace{0.2cm}

\textbf{Substitute:} $\heat{Q} = (2.0\text{ kg})(4186\text{ J/kg·C})(60\text{ C})$

\pause
\vspace{0.2cm}

$$\heat{Q} = 502,320\text{ J} = \boxed{502\text{ kJ}}$$
...
\note{[Fig 11.8: Rock/water/oxygen showing particle spacing] "Concrete visual - tightly bound solid, attracted liquid, bouncing gas - use to discuss freedom vs binding energy"\\\\
[P0] "Four states of matter in order of energy"\\\\
...
\note{[Fig 11.10: Heating curve - temp vs heat, plateaus at 0C and 100C] "Critical graph - trace with finger while explaining each segment - students must internalize that adding heat doesn't always raise temp"\\\\
[P0] "Graph shows heating ice at -20 C until it becomes steam"\\\\
...
\begin{align*}
\heat{Q} &= \mass{m}\specheat{L_f} \quad \text{(melting/freezing)} \\
\heat{Q} &= \mass{m}\specheat{L_v} \quad \text{(vaporization/condensation)}
\end{align*}
\end{block}
...
\textbf{For water:}
\begin{itemize}
\item $\specheat{L_f} = 334$ kJ/kg (latent \heat{heat} of fusion)
\item $\specheat{L_v} = 2256$ kJ/kg (latent \heat{heat} of vaporization)
\end{itemize}

\pause
\vspace{0.3cm}

\alert{Note: No $\Delta \temp{T}$ in these equations - \temp{temperature} doesn't change!}
...
\textbf{Key insight:} Phase change, so use $\heat{Q} = \mass{m}\specheat{L_f}$

\pause
\vspace{0.2cm}

\textbf{Substitute:} $\heat{Q} = (0.50\text{ kg})(334\text{ kJ/kg})$

\pause
\vspace{0.2cm}

$$\heat{Q} = \boxed{167\text{ kJ}}$$
...
\begin{enumerate}
\item \temp{Temperature} = average molecular kinetic energy \pause
\item \heat{Heat} = energy transfer from hot to cold \pause
\item $\heat{Q} = \mass{m}\specheat{c}\Delta \temp{T}$ for \temp{temperature} changes \pause
\item Three transfer modes: conduction, convection, radiation \pause
\item $\heat{Q} = \mass{m}\specheat{L}$ for phase changes (no \temp{temp} change!) \pause
\item Water's high \specheat{specific heat} stabilizes climate \pause
...
\textbf{\temp{Temperature} Conversions:}
\begin{align}
\temp{T_{\circ F}} &= \frac{9}{5}\temp{T_{\circ C}} + 32 \\
\temp{T_{\circ C}} &= \frac{5}{9}(\temp{T_{\circ F}} - 32) \\
\temp{T_{K}} &= \temp{T_{\circ C}} + 273.15
...
\textbf{\heat{Heat} Transfer:}
\begin{align}
\heat{Q} &= \mass{m}\specheat{c}\Delta \temp{T} \quad \text{(\temp{temperature} change)} \\
\heat{Q} &= \mass{m}\specheat{L_f} \quad \text{(melting/freezing)} \\
\heat{Q} &= \mass{m}\specheat{L_v} \quad \text{(vaporization/condensation)}
\end{align}
...
\begin{itemize}
\item $\specheat{c_{water}} = 4186$ J/kg·C
\item $\specheat{L_f} = 334$ kJ/kg
\item $\specheat{L_v} = 2256$ kJ/kg
\end{itemize}
