...
\note{[P0][Fig 12.1: Smoke from steam train in mountains] "Steam engines revolutionized civilization"\\\\
...
[TEACHING HINT] Visible smoke represents wasted thermal energy - connects abstract efficiency to visible loss}
\end{frame}
...
\caption{Engineer monitoring thermal systems}
\end{figure}

\pause
\textbf{Zeroth law in action:}\\
Air, incubator walls, and baby all reach thermal equilibrium at safe temperature.
\note{[Fig 12.2: Engineer monitoring thermal control systems] "Premature babies have little body covering"\\\\
...
[TEACHING HINT] Connect to real medical tech - students see abstraction becoming life-saving}
\end{frame}
...
\end{alertblock}
\note{[P0][Fig 12.3: Finger vs needle with same force, different pressure] "Same force applied two ways"\\\\
...
[TEACHING HINT] Ask students which hurts more - then reveal same force. Visceral experience anchors P=F/A}
\end{frame}
...
\begin{center}
\Large $\boxed{\pressure{P}\volume{V} = \particles{N}\boltz{k}\temp{T}}$
\end{center}
\pressure{Pressure} times \volume{volume} equals \particles{particles} times \boltz{Boltzmann constant} times \temp{absolute temperature}.
\end{block}
...
\item $\boltz{k} = 1.38 \times 10^{-23}$ J/K (\boltz{Boltzmann constant})
...
\begin{itemize}
\item \textbf{Constant \volume{volume}:} $\pressure{P} \propto \temp{T}$ (\pressure{pressure} rises with \temp{temperature}) \pause
\item \textbf{Constant \temp{temperature}:} $\pressure{P} \propto \frac{1}{\volume{V}}$ (\pressure{pressure} inverse to \volume{volume}) \pause
\item \textbf{Constant \pressure{pressure}:} $\volume{V} \propto \temp{T}$ (\volume{volume} rises with \temp{temperature})
\end{itemize}
...
\pause
(a) \volume{Volume} increases. (b) \pressure{Pressure} increases. (c) \temp{Temperature} increases.
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.45\textheight,keepaspectratio]{phys11-thermodynamics-fig12-5.jpg}
...
\end{block}
\note{[P0][Fig 12.5: Piston moves distance d as energy added] "When gas expands, it does work"\\\\
...
[TEACHING HINT] Draw connection: linear F×d becomes volumetric P×ΔV}
\end{frame}
...
\begin{center}
\Large $\boxed{\Delta \internal{U} = \heat{Q} - \work{W}}$
\end{center}
Change in \internal{internal energy} equals \heat{heat} added minus \work{work} done by system.
\end{block}
...
Where:
\begin{itemize}
\item $\Delta \internal{U}$ = change in \internal{internal energy}
\item $\heat{Q}$ = net \heat{heat} into system (positive if in, negative if out)
\item $\work{W}$ = net \work{work} by system (positive if out, negative if in)
\end{itemize}
...
\note{[P0][Fig 12.6: Q adds energy, W removes energy] [ALGEBRA] "Delta U equals Q minus W"\\\\
...
[TEACHING HINT] Use bank account analogy: deposits-withdrawals=balance change}
\end{frame}
...
Positive $\heat{Q}$ adds energy. Positive $\work{W}$ removes energy.
\end{alertblock}
...
$\heat{Q}$ in adds energy. $\work{W}$ out removes energy. $\Delta \internal{U}$ is net change.
\note{[Fig 12.8: Two processes produce same ΔU=9J via different Q,W paths] "System like bank account"\\\\
...
[TEACHING HINT] Emphasize state function: final ΔU same regardless of path taken}
\end{frame}
...
\textbf{Find:} Net change in \internal{internal energy} $\Delta \internal{U}$
...
\begin{enumerate}
\item What was net \heat{heat} $\heat{Q}$? How calculate?
\item What was net \work{work} $\work{W}$? Signs correct?
\item Did you use $\Delta \internal{U} = \heat{Q} - \work{W}$?
\end{enumerate}
...
\textbf{Step 1 - Net \heat{heat}:}
$$\heat{Q} = 40.0 \text{ J} - 25.0 \text{ J} = 15.0 \text{ J}$$

\pause
\vspace{0.2cm}

\textbf{Step 2 - Net \work{work}:}
$$\work{W} = 10.0 \text{ J} - 4.0 \text{ J} = 6.0 \text{ J}$$

\pause
\vspace{0.2cm}

\textbf{Step 3 - First law:}
$$\Delta \internal{U} = \heat{Q} - \work{W} = 15.0 \text{ J} - 6.0 \text{ J}$$

\pause
$$\boxed{\Delta \internal{U} = 9.0 \text{ J}}$$

\pause
\textbf{Check:} More \heat{heat} in than \work{work} out $\rightarrow$ \internal{internal energy} increases.
...
\pause
$$\Delta \internal{U} = \heat{Q} - \work{W} + \text{food energy}$$
...
\end{itemize}
\note{[P0][Fig 12.7: Person takes in food, gives off heat, does work; flower absorbs sunlight] "Your body obeys first law"\\\\
...
[TEACHING HINT] Weight gain/loss is thermodynamics - energy balance determines ΔU (fat storage)}
\end{frame}
...
\end{center}
Change in \entropy{entropy} equals \heat{heat} transfer divided by \temp{absolute temperature}.
\end{block}

\pause
\vspace{0.3cm}

\textbf{What \entropy{entropy} measures:}
\begin{itemize}
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys11-thermodynamics-fig12-12.jpg}
...
\end{itemize}
\note{[P0][Fig 12.12: Ice structure vs water disorder, positive entropy change] "Ice melting: classic entropy increase"\\\\
...
[TEACHING HINT] Visual comparison makes abstract entropy concrete - show molecular arrangements}
\end{frame}
...
$$\Delta \entropy{S}_{\text{total}} = \Delta \entropy{S}_{\text{system}} + \Delta \entropy{S}_{\text{env}} \geq 0$$
...
\begin{itemize}
\item \heat{Heat} flows spontaneously hot to cold, never cold to hot
\item Energy becomes less available over time
\item Disorder increases
\end{itemize}
\end{alertblock}
\note{[P0][Fig 12.10: Heat hot→cold, brake heat, gas expansion all irreversible] "Second law - entropy never decreases"\\\\
...
[TEACHING HINT] All three panels show irreversibility - nature's one-way processes}
\end{frame}
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys11-thermodynamics-fig12-10.jpg}
\end{figure}

\pause
\textbf{Why \heat{heat} flows hot to cold:}\\
Larger \entropy{entropy} increase at low \temp{T} than decrease at high \temp{T}.

\pause
$$\Delta \entropy{S} = \frac{\heat{Q}}{\temp{T}} \rightarrow \text{smaller } \temp{T} \text{ means larger } \Delta \entropy{S}$$
\note{[P0][Fig 12.10: Heat transfers hot→cold, irreversible processes] "Heat always flows hot to cold - why?"\\\\
...
[TEACHING HINT] Use numerical example: same Q at 300K vs 600K shows asymmetric ΔS}
\end{frame}
...
\note{[P0][Fig 12.11: Ice frozen in alligator shape emerging from water] "Can entropy decrease? Yes, locally, with energy input"\\\\
...
[TEACHING HINT] Ice sculpture shows local order requires energy input from environment}
\end{frame}
...
\textbf{Step 1 - \heat{Heat} to melt:}
$$\heat{Q} = \mass{m}\specheat{L_f} = (1.00)(334 \text{ kJ/kg}) = 3.34 \times 10^5 \text{ J}$$

\pause
\vspace{0.2cm}

\textbf{Step 2 - Convert \temp{temp}:}
$$\temp{T} = 0^\circ\text{C} = 273 \text{ K}$$

\pause
\vspace{0.2cm}

\textbf{Step 3 - \entropy{Entropy}:}
$$\Delta \entropy{S} = \frac{\heat{Q}}{\temp{T}} = \frac{3.34 \times 10^5}{273}$$

\pause
$$\boxed{\Delta \entropy{S} = 1.22 \times 10^3 \text{ J/K}}$$
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys11-thermodynamics-fig12-14.jpg}
\end{figure}

\pause
\begin{enumerate}
\item Absorb \heat{heat} $\heat{Q_h}$ from hot reservoir
\item Do \work{work} $\work{W}$
\item Reject waste \heat{heat} $\heat{Q_c}$ to cold reservoir
\end{enumerate}

\pause
$$\work{W} = \heat{Q_h} - \heat{Q_c}$$
\note{[P0][Fig 12.14: Heat Qh from hot reservoir, work W output, waste Qc to cold] "Heat engine operation"\\\\
...
[TEACHING HINT] Trace energy flow: Qh splits into useful W and unavoidable waste Qc}
\end{frame}
...
\end{center}
Efficiency equals useful \work{work} divided by \heat{heat} input.
\end{block}
...
\note{[P0][Fig 12.14: W=Qh-Qc shows unavoidable waste] [ALGEBRA] "Eff = W/Q_h"\\\\
...
[TEACHING HINT] Calculate max efficiency W/Qh=(Qh-Qc)/Qh requires Qc>0 always}
\end{frame}

\begin{frame}
\frametitle{12.4 Why Engines Cannot Be Perfect}
\textbf{Perfect efficiency requires:} $\heat{Q_c} = 0$ (no waste \heat{heat})
...
\begin{itemize}
\item \entropy{Entropy} must increase
\item \heat{Heat} MUST flow to cold reservoir
...
\textbf{(a) \work{Work} output:}
$$\work{W} = \heat{Q_h} - \heat{Q_c} = 2.50 \times 10^{14} - 1.48 \times 10^{14}$$

\pause
$$\boxed{\work{W} = 1.02 \times 10^{14} \text{ J}}$$

\pause
\vspace{0.3cm}

\textbf{(b) Efficiency:}
$$\text{Eff} = \frac{\work{W}}{\heat{Q_h}} = \frac{1.02 \times 10^{14}}{2.50 \times 10^{14}}$$
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys11-thermodynamics-fig12-15.jpg}
\end{figure}

\pause
\textbf{\heat{Heat} engines in reverse:}
\begin{itemize}
\item Use \work{work} to move \heat{heat} cold to hot
\item Refrigerators cool interior, warm exterior
\item \heat{Heat} pumps warm house using outside air
\end{itemize}
\note{[P0][Fig 12.15: Complete heat pump cycle - evaporator, compressor, condenser, valve] "Heat pumps/fridges: engines backward"\\\\
...
[TEACHING HINT] Trace refrigerant cycle: absorbs Qc outdoors, work input, releases Qh indoors}
\end{frame}
...
\begin{enumerate}
\item \textbf{Zeroth:} \temp{Temperature} equilibrium transitive \pause
\item \textbf{First:} Energy conserved: $\Delta \internal{U} = \heat{Q} - \work{W}$ \pause
\item \textbf{Second:} \entropy{Entropy} always increases \pause
...
\begin{align}
\pressure{P} &= \frac{\force{F}}{\area{A}} \quad \text{(\pressure{Pressure})} \\
\pressure{P}\volume{V} &= \particles{N}\boltz{k}\temp{T} \quad \text{(Ideal gas)} \\
\work{W} &= \pressure{P}\Delta \volume{V} \quad \text{(\pressure{P}-\volume{V} work)} \\
\Delta \internal{U} &= \heat{Q} - \work{W} \quad \text{(First law)} \\
\Delta \entropy{S} &= \frac{\heat{Q}}{\temp{T}} \quad \text{(\entropy{Entropy})} \\
\text{Eff} &= \frac{\work{W}}{\heat{Q_h}} \quad \text{(Efficiency)}
\end{align}
