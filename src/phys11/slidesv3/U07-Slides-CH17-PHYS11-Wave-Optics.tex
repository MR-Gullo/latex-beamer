...
\begin{itemize}
\item \vel{Speed}: $\vel{c} = 3.00 \times 10^8$ m/s (constant) \pause
\item \wavelen{Wavelength}: $\wavelen{\lambda} = 380$ to 750 nm \pause
\item \freq{Frequency}: $\freq{f} = 4.0 \times 10^{14}$ to $7.9 \times 10^{14}$ Hz
\end{itemize}
...
\begin{itemize}
\item \vel{Speed} changes: $\vel{v} = \frac{\vel{c}}{\pConst{n}}$ \pause
\item \wavelen{Wavelength} changes: $\wavelen{\lambda_n} = \frac{\wavelen{\lambda}}{\pConst{n}}$ \pause
\item \freq{Frequency} stays constant \pause
\item Color stays constant (color linked to \freq{frequency})
\end{itemize}
...
Water has $\pConst{n} = 1.333$, so visible \wavelen{wavelengths} compress to 285-570 nm
\end{exampleblock}
...
\textbf{Answer:} \wavelen{Wavelength} compared to opening size
\begin{itemize}
\item Sound \wavelen{wavelength}: $\sim$1 m (comparable to door width) \pause
\item Visible light \wavelen{wavelength}: $\sim$500 nm (much smaller than door)
\end{itemize}

\pause
\begin{alertblock}{The Paradox}
Light DOES bend - but only around objects comparable to its \wavelen{wavelength}
\end{alertblock}
...
\begin{center}
\Large $\boxed{\disp{d} \sin \angle{\theta} = m\wavelen{\lambda}}$
\end{center}
...
\begin{center}
\Large $\boxed{\disp{d} \sin \angle{\theta} = \left(m + \frac{1}{2}\right)\wavelen{\lambda}}$
\end{center}
...
\caption{Path difference $\Delta \disp{L} = \disp{d} \sin \angle{\theta}$}
\end{figure}
...
\begin{center}
$\boxed{\disp{D} \sin \angle{\theta} = m\wavelen{\lambda}}$ \quad or \quad $\boxed{\frac{\disp{D}\disp{y}}{\disp{L}} = m\wavelen{\lambda}}$
\end{center}
...
\textbf{Given:}
\begin{itemize}
\item $\disp{d} = 0.0100$ mm $= 1.00 \times 10^{-5}$ m
\item $\angle{\theta} = 10.95^\circ$
\item $m = 3$ (third bright line)
\end{itemize}

\textbf{Find:} \wavelen{Wavelength} $\wavelen{\lambda}$ in nm
...
\item How did you rearrange it to solve for $\wavelen{\lambda}$?
\item What units did you get for \wavelen{wavelength}?
\end{enumerate}
...
\textbf{Equation:} $\disp{d} \sin \angle{\theta} = m\wavelen{\lambda}$

\pause
\vspace{0.2cm}

\textbf{Rearrange:} $\wavelen{\lambda} = \frac{\disp{d} \sin \angle{\theta}}{m}$

\pause
\vspace{0.2cm}

\textbf{Substitute:} $\wavelen{\lambda} = \frac{(1.00 \times 10^{-5} \text{ m})(\sin 10.95^\circ)}{3}$

\pause
\vspace{0.2cm}

$$\wavelen{\lambda} = \frac{(1.00 \times 10^{-5})(0.190)}{3} = 6.33 \times 10^{-7} \text{ m}$$

\pause
$$\boxed{\wavelen{\lambda} = 633 \text{ nm}}$$

\pause
\textbf{Check:} 633 nm is red light - \wavelen{wavelength} of He-Ne laser. Perfect!
...
\textbf{Given:}
\begin{itemize}
\item $\wavelen{\lambda} = 550$ nm $= 550 \times 10^{-9}$ m
\item $\angle{\theta} = 45.0^\circ$
\item $m = 2$ (second minimum)
\end{itemize}

\textbf{Find:} Slit width $\disp{D}$ in micrometers
...
\item How did you solve for $\disp{D}$?
...
\textbf{Rearrange:} $\disp{D} = \frac{m\wavelen{\lambda}}{\sin \angle{\theta}}$

\pause
\vspace{0.2cm}

\textbf{Substitute:} $\disp{D} = \frac{2(550 \times 10^{-9} \text{ m})}{\sin 45.0^\circ}$

\pause
\vspace{0.2cm}

$$\disp{D} = \frac{1100 \times 10^{-9}}{0.707} = 1.56 \times 10^{-6} \text{ m}$$

\pause
$$\boxed{\disp{D} = 1.56 \text{ }\mu\text{m}}$$

\pause
\textbf{Check:} Only few times \wavelen{wavelength} - consistent with significant wave effects!
\note{[P0] "Self-correct in different color"\\\\
...
\pause
$$\boxed{\angle{\theta} = 1.22 \frac{\wavelen{\lambda}}{\disp{D}}}$$
where $\angle{\theta}$ is minimum resolvable \angle{angle} (in radians), $\disp{D}$ is aperture diameter
...
\textbf{Given:}
\begin{itemize}
\item $\wavelen{\lambda} = 550$ nm (in vacuum/air)
\item $\pConst{n} = 1.33$ (water)
\end{itemize}

\textbf{Find:} \wavelen{Wavelength} $\wavelen{\lambda_n}$ in water
...
\begin{enumerate}
\item What happens to \vel{speed}, \wavelen{wavelength}, and \freq{frequency} when light enters water?
\item Which equation relates \wavelen{wavelength} in medium to \wavelen{wavelength} in vacuum?
\item Does \wavelen{wavelength} increase or decrease in water?
\end{enumerate}
...
\textbf{Equation:} $\wavelen{\lambda_n} = \frac{\wavelen{\lambda}}{\pConst{n}}$

\pause
\vspace{0.2cm}

\textbf{Substitute:} $\wavelen{\lambda_n} = \frac{550 \text{ nm}}{1.33}$

\pause
\vspace{0.2cm}

$$\boxed{\wavelen{\lambda_n} = 414 \text{ nm}}$$

\pause
\vspace{0.3cm}

\textbf{Check:} \wavelen{Wavelength} decreased (550 $\rightarrow$ 414 nm). Color stays green because \freq{frequency} constant!
\note{[P0] "Self-correct in different color"\\\\
...
\textbf{Given:}
\begin{itemize}
\item 2,000 lines/cm $\rightarrow$ $\disp{d} = \frac{1 \text{ cm}}{2000} = 5.00 \times 10^{-4}$ cm
\item $\wavelen{\lambda} = 520$ nm $= 520 \times 10^{-9}$ m
\item $m = 1$ (first-order maximum)
\end{itemize}

\textbf{Find:} \angle{Angle} $\angle{\theta}$ for first-order maximum
...
\begin{enumerate}
\item How did you calculate $\disp{d}$ from lines per cm?
\item Which equation relates $\disp{d}$, $\angle{\theta}$, and $\wavelen{\lambda}$ for grating?
\item How did you solve for $\angle{\theta}$?
\end{enumerate}
...
\textbf{Find d:} $\disp{d} = \frac{1 \text{ cm}}{2000} = 5.00 \times 10^{-4}$ cm $= 5.00 \times 10^{-6}$ m
...
\textbf{Rearrange:} $\angle{\theta} = \sin^{-1}\left(\frac{m\wavelen{\lambda}}{\disp{d}}\right)$

\pause
\vspace{0.2cm}

\textbf{Substitute:} $\angle{\theta} = \sin^{-1}\left(\frac{(1)(520 \times 10^{-9})}{5.00 \times 10^{-6}}\right) = \sin^{-1}(0.104)$

\pause
$$\boxed{\angle{\theta} = 5.97^\circ}$$

\pause
\textbf{Check:} Small \angle{angle} - reasonable for first maximum!
\note{[P0] "Self-correct in different color"\\\\
...
\textbf{Given:}
\begin{itemize}
\item $\wavelen{\lambda} = 633$ nm $= 633 \times 10^{-9}$ m
\item $\disp{D} = 1.00$ mm $= 1.00 \times 10^{-3}$ m
\end{itemize}

\textbf{Find:} Minimum angular spread $\angle{\theta}$ in radians and degrees
...
\item What does the diameter $\disp{D}$ represent?
...
\textbf{Equation:} $\angle{\theta} = \frac{1.22\wavelen{\lambda}}{\disp{D}}$

\pause
\vspace{0.2cm}

\textbf{Substitute:} $\angle{\theta} = \frac{(1.22)(633 \times 10^{-9} \text{ m})}{1.00 \times 10^{-3} \text{ m}}$

\pause
\vspace{0.2cm}

$$\angle{\theta} = 7.72 \times 10^{-4} \text{ rad}$$

\pause
\vspace{0.2cm}

\textbf{Convert:} $\angle{\theta} = (7.72 \times 10^{-4})(57.3^\circ/\text{rad}) = \boxed{0.0442^\circ}$

\pause
\textbf{Check:} Tiny spread - barely noticeable over short \disp{distances}!
\note{[P0] "Self-correct in different color"\\\\
...
\item Double-slit interference proves wave nature: $\disp{d} \sin \angle{\theta} = m\wavelen{\lambda}$ \pause
\item Single slit produces diffraction pattern: $\disp{D} \sin \angle{\theta} = m\wavelen{\lambda}$ \pause
\item Lasers produce coherent light via stimulated emission \pause
\item Diffraction gratings separate \wavelen{wavelengths} sharply \pause
\item Resolution fundamentally limited by wave nature: $\angle{\theta} = 1.22\wavelen{\lambda}/\disp{D}$ \pause
\item \wavelen{Wavelength} changes in media: $\wavelen{\lambda_n} = \wavelen{\lambda}/\pConst{n}$, but \freq{frequency} constant
\end{enumerate}
\end{block}
...
\begin{align}
\vel{c} &= \freq{f}\wavelen{\lambda} \quad \text{(light in vacuum)} \\
\wavelen{\lambda_n} &= \frac{\wavelen{\lambda}}{\pConst{n}} \quad \text{(wavelength in medium)} \\
\disp{d} \sin \angle{\theta} &= m\wavelen{\lambda} \quad \text{(double-slit constructive)} \\
\disp{d} \sin \angle{\theta} &= \left(m + \frac{1}{2}\right)\wavelen{\lambda} \quad \text{(double-slit destructive)} \\
\disp{D} \sin \angle{\theta} &= m\wavelen{\lambda} \quad \text{(single-slit minima)} \\
\angle{\theta} &= 1.22\frac{\wavelen{\lambda}}{\disp{D}} \quad \text{(Rayleigh criterion)}
\end{align}
