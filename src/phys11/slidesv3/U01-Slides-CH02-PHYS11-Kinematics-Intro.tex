...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys11-kinematics-fig2-2.jpg}
...
\note{[P0][Fig 2.2: Airplane flying upward past clouds] "Are clouds a useful reference frame?"\\\\
[P1] "No! Clouds move with the plane"\\\\
[THE CONFLICT] Your intuition says motion is obvious. Physics says: compared to what?\\\\
- Teaching hint: Ask students what happens when they look out airplane window - clouds appear stationary but they're moving too\\\\
...
\begin{center}
\includegraphics[width=0.5\textwidth,height=0.4\textheight,keepaspectratio]{phys11-kinematics-fig2-3.jpg}
...
\note{[P0][Fig 2.3: Painting of Galileo Galilei] "Galileo - one of the first to explore reference frames"\\\\
...
- Teaching hint: Have students close eyes and imagine being in smooth elevator - impossible to know if moving without external reference\\\\
...
\begin{block}{\disp{Distance}}
Total length of the path traveled. (Scalar)
\end{block}

\pause

\begin{block}{\disp{Displacement}}
Straight-line change in \pos{position} from start to finish. (Vector)
\end{block}
...
\textbf{Physicist:} "But your \disp{displacement} was 0 - you ended where you started."
\end{alertblock}
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys11-kinematics-fig2-5.jpg}
\caption{Short line is displacement. Curved path is distance.}
\end{figure}

\pause
\disp{Distance} $\geq$ \disp{Displacement} (always!)
\note{[P0][Fig 2.5: House and friend's house with curvy road] "Visual representation of distance vs displacement"\\\\
[P1] "Short straight arrow between houses is displacement magnitude"\\\\
- "Curved road length is distance traveled"\\\\
- Distance is always greater than or equal to displacement magnitude\\\\
- Teaching hint: Ask students which they'd use for fuel consumption (distance) vs helicopter flight (displacement)\\\\
...
\pause
\textbf{\disp{Distance}:} 10 km

\pause
\textbf{\disp{Displacement}:} 0 km (start and end at same \pos{position})
\note{[P0][Fig 2.6: House and school 5 mi apart, round trip] "Classic example: drive to school and back"\\\\
...
- Teaching hint: Ask students about their commute - total distance for week vs displacement for week (both zero if they return home)}
\end{frame}
...
\begin{block}{Calculating \disp{Displacement}}
\begin{center}
\Large $\boxed{\Delta \disp{\mathbf{d}} = \disp{\mathbf{d}}_{\mathrm{f}} - \disp{\mathbf{d}}_{0}}$
\end{center}
Change in \pos{position} equals final \pos{position} minus initial \pos{position}.
\end{block}
...
\item $\disp{\mathbf{d}}_{0}$ = initial \pos{position}
\item $\disp{\mathbf{d}}_{\mathrm{f}}$ = final \pos{position}
\end{itemize}
...
\textbf{(a) \disp{Displacement}:}
$$\Delta \disp{\mathbf{d}} = \disp{\mathbf{d}}_{\mathrm{f}} - \disp{\mathbf{d}}_{0} = -1 \text{ km}$$
(or 1 km west)

\pause

\textbf{(b) \disp{Distance} traveled:}
$$\text{\disp{Distance}} = 3 \text{ km} + 2 \text{ km} = 5 \text{ km}$$

\pause

\textbf{(c) Magnitude of \disp{displacement}:}
$$|\Delta \disp{\mathbf{d}}| = 1 \text{ km}$$
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth,height=0.45\textheight,keepaspectratio]{phys11-kinematics-fig2-7.jpg}
...
\note{[P0][Fig 2.7: Mars Climate Orbiter above Mars] "Real-world consequence of unit mistakes"\\\\
...
- Teaching hint: Show how 1 meter vs 1 foot error compounds over millions of km - ask students to calculate the cumulative error\\\\
...
\vel{Speed} is the rate at which an object changes its location. (Scalar)
\end{block}
...
\begin{center}
\Large $\boxed{\vel{v}_{\text{avg}} = \frac{\disp{d}}{\tvar{t}}}$
\end{center}
Average \vel{speed} equals total \disp{distance} divided by total \tvar{time}.
\end{block}
...
\textbf{Rearranged:}
\begin{align*}
\disp{d} &= \vel{v}_{\text{avg}} \times \tvar{t}\\
\tvar{t} &= \frac{\disp{d}}{\vel{v}_{\text{avg}}}
\end{align*}
...
\textbf{Average \vel{speed}:} Total \disp{distance} divided by total \tvar{time}

\pause
\vspace{0.3cm}

\textbf{Example:} Car travels 150 km in 3.2 hours
$$\vel{v}_{\text{avg}} = \frac{150 \text{ km}}{3.2 \text{ h}} = 47 \text{ km/h}$$

\pause
\vspace{0.3cm}

\textbf{Instantaneous \vel{speed}:} \vel{Speed} at a specific instant
...
\pause
\textbf{Average \vel{speed}:} $\frac{6 \text{ km}}{0.5 \text{ h}} = 12 \text{ km/h}$

\textbf{\disp{Displacement}:} 0 km (round trip)
\note{[P0][Fig 2.8: House and store 3 km apart, round trip] "Round trip example"\\\\
...
- Teaching hint: This is why jogging apps show total distance run (speed calculation) but finish where you started (zero displacement, zero average velocity)}
\end{frame}
...
\textbf{Substitute:}
$$\vel{v}_{\text{avg}} = \frac{5.2 \text{ m}}{1.8 \text{ s}} = 2.9 \text{ m/s}$$
...
\begin{block}{\vel{Speed} Plus Direction}
\vel{Velocity} is the vector version of \vel{speed}. It includes both magnitude (how fast) and direction (which way).
\end{block}
...
\begin{exampleblock}{The Mental Model}
\vel{Velocity} is your speedometer with a compass attached.\\
\vel{Speed} tells you how fast. \vel{Velocity} adds where.
\end{exampleblock}
...
\begin{itemize}
\item $\vel{\mathbf{v}}$ = \vel{velocity} (vector, includes direction)
\item $\vel{v}$ = \vel{speed} (scalar, magnitude only)
\end{itemize}
...
\begin{block}{Universal Law: Rate of \disp{Displacement}}
\begin{center}
\Large $\boxed{\vel{\mathbf{v}}_{\text{avg}} = \frac{\Delta \disp{\mathbf{d}}}{\Delta \tvar{t}} = \frac{\disp{\mathbf{d}}_{\mathrm{f}} - \disp{\mathbf{d}}_{0}}{\tvar{t}_{\mathrm{f}} - \tvar{t}_{0}}}$
\end{center}
Average \vel{velocity} equals \disp{displacement} divided by \tvar{time} interval.
\end{block}

\pause
\vspace{0.3cm}

\textbf{SI unit:} m/s (same as \vel{speed}, but includes direction)

\pause
\vspace{0.3cm}

\begin{alertblock}{The Paradox}
Round trip: \vel{Speed} = 12 km/h, but \vel{velocity} = 0 km/h!\\
(\disp{Displacement} is zero)
\end{alertblock}
...
\begin{columns}[T]
\column{0.48\textwidth}
\begin{block}{\vel{Speed} (Scalar)}
\begin{itemize}
\item Uses \disp{distance}
\item Magnitude only
...
\pause
\column{0.48\textwidth}
\begin{block}{\vel{Velocity} (Vector)}
\begin{itemize}
\item Uses \disp{displacement}
...
\textbf{Civilian:} "\vel{Speed} and \vel{velocity} are the same thing."\\
\textbf{Physicist:} "\vel{Speed} is how fast. \vel{Velocity} is how fast AND which way."
\end{alertblock}
...
\textbf{Equation:}
$$\vel{\mathbf{v}}_{\text{avg}} = \frac{\Delta \disp{\mathbf{d}}}{\Delta \tvar{t}}$$

\pause

\textbf{Substitute:}
$$\vel{\mathbf{v}}_{\text{avg}} = \frac{304 \text{ m}}{180 \text{ s}} = 1.7 \text{ m/s north}$$
...
\textbf{Average \vel{velocity}:} Over a \tvar{time} interval

\pause
\vspace{0.3cm}

\textbf{Instantaneous \vel{velocity}:} At a specific instant in \tvar{time}
...
\pause
If \vel{velocity} is constant, instantaneous = average.
\note{[P0][Fig 2.9: Airplane with displacement vectors in segments] "Two types of velocity, like speed"\\\\
[P1] "Average: over entire time interval"\\\\
[P2] "Instantaneous: at one specific moment"\\\\
- Diagram shows smaller time segments a, b, c, d\\\\
[P3] "Special case: constant velocity means instantaneous equals average throughout"\\\\
- Teaching hint: Ask which segment has different direction (c points right while others point left) - velocity changed because direction changed\\\\
...
\item \disp{Distance} (scalar) vs. \disp{displacement} (vector) \pause
\item \vel{Speed} (scalar) vs. \vel{velocity} (vector) \pause
\item \disp{Displacement} = final \pos{position} - initial \pos{position} \pause
\item Average \vel{speed} = \disp{distance} / \tvar{time} \pause
\item Average \vel{velocity} = \disp{displacement} / \tvar{time}
\end{enumerate}
\end{block}
...
\begin{align}
\Delta \disp{\mathbf{d}} &= \disp{\mathbf{d}}_{\mathrm{f}} - \disp{\mathbf{d}}_{0} \\
\vel{v}_{\text{avg}} &= \frac{\disp{d}}{\tvar{t}} \\
\vel{\mathbf{v}}_{\text{avg}} &= \frac{\Delta \disp{\mathbf{d}}}{\Delta \tvar{t}} = \frac{\disp{\mathbf{d}}_{\mathrm{f}} - \disp{\mathbf{d}}_{0}}{\tvar{t}_{\mathrm{f}} - \tvar{t}_{0}}
\end{align}
