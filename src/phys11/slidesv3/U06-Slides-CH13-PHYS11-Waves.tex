...
\textbf{\amplitude{Amplitude}} $\amplitude{A}$: Maximum displacement from equilibrium\\
\textbf{\wavelen{Wavelength}} $\wavelen{\lambda}$: Distance between adjacent crests\\
\textbf{\period{Period}} $\period{T}$: Time for one complete cycle\\
\textbf{\freq{Frequency}} $\freq{f}$: Number of cycles per second (Hz)\\
\textbf{\vel{Wave velocity}} $\vel{v_w}$: Speed of disturbance
\end{block}
...
\pause
\begin{itemize}
\item $\wavelen{\lambda}$ = \wavelen{wavelength} (crest to crest)
\item $\amplitude{A}$ = \amplitude{amplitude} (rest to crest)
\item $\vel{v_w}$ = \vel{wave velocity} (disturbance speed)
\end{itemize}
...
\begin{center}
\Large $\boxed{\freq{f} = \frac{1}{\period{T}}}$
\end{center}
\freq{Frequency} equals one over \period{period}.
\end{block}
...
\textbf{Meaning:}
\begin{itemize}
\item Higher \freq{frequency} $\rightarrow$ shorter \period{period} \pause
\item Lower \freq{frequency} $\rightarrow$ longer \period{period}
\end{itemize}
...
\begin{center}
\Large $\boxed{\vel{v_w} = \freq{f} \wavelen{\lambda}}$
\end{center}
\vel{Wave velocity} equals \freq{frequency} times \wavelen{wavelength}.
\end{block}
...
$$\vel{v_w} = \frac{\wavelen{\lambda}}{\period{T}}$$

\pause
\textbf{Key insight:}
In a given medium where $\vel{v_w}$ is constant, higher \freq{frequency} means shorter \wavelen{wavelength}.
...
\textbf{Woofer} (large): low \freq{frequency}, long \wavelen{wavelength}\\
\textbf{Tweeter} (small): high \freq{frequency}, short \wavelen{wavelength}
\end{exampleblock}
...
\item Energy related to \amplitude{amplitude} - large \amplitude{amplitude} = more damage
\end{itemize}
...
\textbf{Given:}
\begin{itemize}
\item $\wavelen{\lambda} = 10.0$ m
\item $\period{T} = 5.00$ s
\end{itemize}

\textbf{Find:} \vel{Wave velocity} $\vel{v_w}$
...
\item Did you use $\vel{v_w} = \freq{f}\wavelen{\lambda}$ or $\vel{v_w} = \frac{\wavelen{\lambda}}{\period{T}}$?
\item If you used \freq{frequency}, how did you find it?
\end{enumerate}
...
\pause
$$\vel{v_w} = \frac{\wavelen{\lambda}}{\period{T}} = \frac{10.0 \text{ m}}{5.00 \text{ s}} = 2.00 \text{ m/s}$$

\pause
\vspace{0.3cm}

\textbf{Method 2:} Using \freq{frequency}
\pause
$$\freq{f} = \frac{1}{\period{T}} = \frac{1}{5.00 \text{ s}} = 0.200 \text{ Hz}$$

\pause
$$\vel{v_w} = \freq{f}\wavelen{\lambda} = (0.200 \text{ Hz})(10.0 \text{ m}) = 2.00 \text{ m/s}$$
...
\textbf{Given:}
\begin{itemize}
\item $\freq{f} = 2$ Hz (2 waves per second)
\item $\wavelen{\lambda} = 0.9$ m (one cycle)
\end{itemize}

\textbf{Find:} (a) \period{Period} $\period{T}$ \quad (b) \vel{Wave velocity} $\vel{v_w}$
...
\begin{enumerate}
\item How did you find the \period{period} from \freq{frequency}?
\item Which \vel{velocity} formula did you use?
...
\textbf{Part (a):} Find \period{period}
\pause
$$\period{T} = \frac{1}{\freq{f}} = \frac{1}{2 \text{ s}^{-1}} = 0.5 \text{ s}$$

\pause
\vspace{0.3cm}

\textbf{Part (b):} Find \vel{wave velocity}
\pause
$$\vel{v_w} = \freq{f}\wavelen{\lambda} = (2 \text{ s}^{-1})(0.9 \text{ m}) = 1.8 \text{ m/s}$$

\pause
\vspace{0.3cm}

\textbf{Check:} Could also use $\vel{v_w} = \frac{\wavelen{\lambda}}{\period{T}} = \frac{0.9}{0.5} = 1.8$ m/s
\note{[P0] "Self-correct in a different color"\\\\
...
\pause
\begin{block}{Nature's Rule}
Constructive interference: waves exactly in phase combine to produce larger \amplitude{amplitude}.
\end{block}

\pause
\amplitude{Amplitude} doubles when two identical waves align crest-to-crest!
...
Two waves can add to create... nothing! Zero \amplitude{amplitude}.
\end{alertblock}
...
\pause
\textbf{Node:} point of zero \amplitude{amplitude} (no motion)

\pause
\textbf{Antinode:} point of maximum \amplitude{amplitude}
...
\pause
\textbf{What changes:} \vel{speed}, \wavelen{wavelength}, direction\\
\textbf{What stays same:} \freq{frequency}
...
\item Five variables: $\amplitude{A}$, $\wavelen{\lambda}$, $\period{T}$, $\freq{f}$, $\vel{v_w}$ \pause
\item Master equation: $\vel{v_w} = \freq{f}\wavelen{\lambda}$ \pause
...
\begin{align}
\freq{f} &= \frac{1}{\period{T}} \quad \text{(\freq{frequency} and \period{period})}\\
\period{T} &= \frac{1}{\freq{f}}\\
\vel{v_w} &= \freq{f}\wavelen{\lambda} \quad \text{(wave equation)}\\
\vel{v_w} &= \frac{\wavelen{\lambda}}{\period{T}}
\end{align}
...
\textbf{Remember:}
\begin{itemize}
\item In constant medium, higher \freq{frequency} $\rightarrow$ shorter \wavelen{wavelength}
\item \amplitude{Amplitude} is independent of \vel{velocity}
