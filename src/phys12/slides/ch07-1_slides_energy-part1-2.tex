...
\begin{frame}
\frametitle{Conservative Forces: The Key Concept}
...
\pause
\textbf{VIDEO:} Pendulum showing continuous KE ↔ PE conversion (30-60 sec)
\end{frame}
...
\begin{center}
\textit{Image suggestion: Energy bar graphs showing KE and PE at different points of motion (e.g., ball thrown upward)}

\textit{Search: "energy bar graph physics" or "kinetic potential energy transformation"}
\end{center}
\pause
...
\begin{itemize}
    ...
    \pause
    \item At lowest point: all KE, no PE (if reference is there)
\end{itemize}

\pause
\textbf{VIDEO SUGGESTION:} Bungee jump showing gravitational PE → KE → elastic PE (45-90 sec)
\end{frame}

\section{7.6 Nonconservative Forces}

\begin{frame}
\frametitle{Nonconservative Forces: The Game Changer}
\begin{block}{Definition}
...
\pause
\vspace{0.5cm}
\textit{Image suggestion: Object sliding with friction arrows and heat indicators}

\textit{Search: "friction force diagram heat"}
\end{column}
...
\pause
\textbf{VIDEO SUGGESTION:} Friction/sliding demonstration showing energy loss as heat (20-40 sec)
\end{frame}

\begin{frame}
\frametitle{Visual Comparison: Conservative vs Nonconservative}
\begin{center}
\textit{Image suggestion: Side-by-side comparison table/diagram}

\textit{Left: Ball rolling on frictionless track (returns to same height)}

\textit{Right: Ball rolling with friction (stops at lower height)}

\textit{Search: "conservative vs nonconservative forces comparison"}
\end{center}
\pause
...
\begin{center}
\textit{Image suggestion: Flow diagram showing mechanical energy → heat energy via friction}

\textit{Search: "energy dissipation friction diagram" or "mechanical energy to thermal energy"}
\end{center}
\pause
...
\begin{figure}
\centering
\textit{Image suggestion: Spring compressed with mass, arrow showing upward launch}

\textit{Search: "spring projectile physics diagram"}
\end{figure}
\end{frame}
...
\pause
h_{\max} &= \frac{v_i^2}{2g} = \frac{(15)^2}{2(9.8)} \\
\pause
h_{\max} &= 11.5 \text{ m}
\end{align*}
\pause
...
\pause
\begin{align*}
\frac{1}{2}mv_i^2 &= \frac{1}{2}mv_f^2 + mg(11.5/2) \\
\pause
v_f^2 &= v_i^2 - 2g(5.75) \\
\pause
v_f &= \sqrt{225 - 112.7} = 10.6 \text{ m/s}
\end{align*}
\end{frame}
...
\begin{center}
\textit{Image suggestion: Block on inclined plane with friction force shown}

\textit{Search: "block sliding ramp friction diagram"}
\end{center}
\end{frame}
