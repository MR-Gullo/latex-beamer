...
\begin{itemize}
\item \disp{Displacement}, \vel{velocity}, \accel{acceleration} \pause
\item \force{Force}, \mom{momentum} \pause
\item Boldface notation: $\mathbf{\force{F}}$, $\mathbf{\vel{v}}$, $\mathbf{\accel{a}}$
\end{itemize}
...
\pause
\textbf{Resultant force:} $\force{F}_{tot} = \sqrt{(400)^2 + (400)^2} = 566$ N
...
\begin{center}
\Large $\boxed{\disp{A_x} = \disp{A} \cos\angle{\theta}}$

\Large $\boxed{\disp{A_y} = \disp{A} \sin\angle{\theta}}$
\end{center}
...
\disp{Displacement}: $\disp{A} = 10.3$ blocks at $\angle{\theta} = 29.1^\circ$ north of east
...
\begin{align*}
\disp{A_x} &= \disp{A} \cos\angle{\theta} \\
...
\begin{align*}
\disp{A_y} &= \disp{A} \sin\angle{\theta} \\
...
\begin{center}
\Large $\boxed{\disp{A} = \sqrt{\disp{A_x}^2 + \disp{A_y}^2}}$

\Large $\boxed{\angle{\theta} = \tan^{-1}\left(\frac{\disp{A_y}}{\disp{A_x}}\right)}$
\end{center}
...
\textbf{Students confuse:} $\mathbf{\disp{A}}_x + \mathbf{\disp{A}}_y = \mathbf{\disp{A}}$ (vector addition)\\
with $\disp{A} = \sqrt{\disp{A_x}^2 + \disp{A_y}^2}$ (magnitude calculation)
\end{alertblock}
...
\begin{enumerate}
\item Find components: $\disp{A_x}, \disp{A_y}, \disp{B_x}, \disp{B_y}$ \pause
\item Add x-components: $\disp{R_x} = \disp{A_x} + \disp{B_x}$ \pause
\item Add y-components: $\disp{R_y} = \disp{A_y} + \disp{B_y}$ \pause
\item Find magnitude: $\disp{R} = \sqrt{\disp{R_x}^2 + \disp{R_y}^2}$ \pause
\item Find direction: $\angle{\theta} = \tan^{-1}(\disp{R_y}/\disp{R_x})$
\end{enumerate}
...
\textbf{Given:}
\begin{itemize}
\item $\disp{A} = 53.0$ m, $\angle{\theta_A} = 20.0^\circ$
\item $\disp{B} = 34.0$ m, $\angle{\theta_B} = 63.0^\circ$
\end{itemize}
...
\textbf{Step 1:} Find components of $\mathbf{\disp{A}}$
\begin{align*}
\disp{A_x} &= (53.0)(\cos 20.0^\circ) = 49.8 \text{ m} \\
\disp{A_y} &= (53.0)(\sin 20.0^\circ) = 18.1 \text{ m}
\end{align*}

\pause

\textbf{Step 2:} Find components of $\mathbf{\disp{B}}$
\begin{align*}
\disp{B_x} &= (34.0)(\cos 63.0^\circ) = 15.4 \text{ m} \\
\disp{B_y} &= (34.0)(\sin 63.0^\circ) = 30.3 \text{ m}
\end{align*}
...
\begin{align*}
\disp{R_x} &= \disp{A_x} + \disp{B_x} = 49.8 + 15.4 = 65.2 \text{ m} \\
\disp{R_y} &= \disp{A_y} + \disp{B_y} = 18.1 + 30.3 = 48.4 \text{ m}
\end{align*}

\pause

\textbf{Step 4:} Find magnitude
$$\disp{R} = \sqrt{(65.2)^2 + (48.4)^2} = \boxed{81.2 \text{ m}}$$

\pause

\textbf{Step 5:} Find direction
$$\angle{\theta} = \tan^{-1}(48.4/65.2) = \boxed{36.6^\circ \text{ north of east}}$$
...
\begin{itemize}
\item No acceleration: $\accel{a_x} = 0$ \pause
\item Constant velocity: $\vel{v_x} = \vel{v_{0x}}$ \pause
\item Position: $\disp{x} = \disp{x_0} + \vel{v_x} \tvar{t}$
\end{itemize}
...
\begin{itemize}
\item Constant acceleration: $\accel{a_y} = -\grav{g} = -9.8$ m/s$^2$ \pause
\item Velocity: $\vel{v_y} = \vel{v_{0y}} - \grav{g}\tvar{t}$ \pause
\item Position: $\disp{y} = \disp{y_0} + \vel{v_{0y}}\tvar{t} - \frac{1}{2}\grav{g}\tvar{t}^2$
\end{itemize}

\pause
\alert{\tvar{Time} $\tvar{t}$ is the only variable connecting the two motions!}
...
$$\disp{d} = \sqrt{\disp{x}^2 + \disp{y}^2}, \quad \angle{\theta} = \tan^{-1}(\disp{y}/\disp{x})$$
$$\vel{v} = \sqrt{\vel{v_x}^2 + \vel{v_y}^2}, \quad \angle{\theta_v} = \tan^{-1}(\vel{v_y}/\vel{v_x})$$
...
\begin{center}
\Large $\boxed{\disp{h} = \frac{\vel{v_{0y}}^2}{2\grav{g}}}$
\end{center}
Maximum height depends only on initial vertical \vel{velocity}.
\end{block}

\pause
\vspace{0.3cm}

\textbf{Key insight:} At maximum height, $\vel{v_y} = 0$
...
\textbf{Given:}
\begin{itemize}
\item $\vel{v_0} = 70.0$ m/s
\item $\angle{\theta} = 75^\circ$
\end{itemize}

\textbf{Find:} Maximum height $\disp{h}$
...
\textbf{Step 1:} Find vertical \vel{velocity} component
\begin{align*}
\vel{v_{0y}} &= \vel{v_0} \sin\angle{\theta} \\
...
\begin{align*}
\disp{h} &= \frac{\vel{v_{0y}}^2}{2\grav{g}} \\
...
\begin{center}
\Large $\boxed{\disp{R} = \frac{\vel{v_0}^2 \sin 2\angle{\theta}}{\grav{g}}}$
\end{center}
\disp{Range} depends on initial \vel{speed} and launch \angle{angle}.
\end{block}
...
\textbf{Key insights:}
\begin{itemize}
\item Maximum range at $\angle{\theta} = 45^\circ$ \pause
...
\textbf{Given:}
\begin{itemize}
\item $\vel{v_0} = 25.0$ m/s, $\angle{\theta} = 35^\circ$
\item $\disp{y} = -20.0$ m
\end{itemize}

\textbf{Find:} \tvar{Time} of flight
...
\begin{enumerate}
\item Which kinematic equation has both $\disp{y}$ and $\tvar{t}$?
\item What values do you substitute for $\disp{y_0}$ and $\disp{y}$?
...
\textbf{Step 1:} Find $\vel{v_{0y}} = (25.0)(\sin 35^\circ) = 14.3$ m/s

\pause

\textbf{Step 2:} Use kinematic equation with $\disp{y_0} = 0$, $\disp{y} = -20.0$ m
$$-20.0 = (14.3)\tvar{t} - (4.90)\tvar{t}^2$$

\pause

\textbf{Step 3:} Rearrange to standard form
$$(4.90)\tvar{t}^2 - (14.3)\tvar{t} - (20.0) = 0$$

\pause

\textbf{Step 4:} Quadratic formula
$$\tvar{t} = \frac{14.3 \pm \sqrt{(14.3)^2 - 4(4.90)(-20.0)}}{2(4.90)}$$
...
$$\tvar{t} = \frac{14.3 + 24.4}{9.8} = 3.96 \text{ s} \quad \text{(physical)}$$
$$\tvar{t} = \frac{14.3 - 24.4}{9.8} = -1.03 \text{ s} \quad \text{(reject)}$$

\pause

$$\boxed{\tvar{t} = 3.96 \text{ s}}$$

\pause
\alert{Negative \tvar{time} means before launch - physically impossible!}
...
\item Maximum height depends only on $\vel{v_{0y}}$
\end{enumerate}
\end{block}
...
\begin{align}
\disp{A} &= \sqrt{\disp{A_x}^2 + \disp{A_y}^2} \\
\angle{\theta} &= \tan^{-1}(\disp{A_y}/\disp{A_x})
\end{align}

\textbf{Projectile Motion:}
\begin{align}
\text{Horizontal: } \disp{x} &= \disp{x_0} + \vel{v_x} \tvar{t} \\
\text{Vertical: } \disp{y} &= \disp{y_0} + \vel{v_{0y}}\tvar{t} - \frac{1}{2}\grav{g}\tvar{t}^2 \\
\text{Max height: } \disp{h} &= \frac{\vel{v_{0y}}^2}{2\grav{g}}
\end{align}
