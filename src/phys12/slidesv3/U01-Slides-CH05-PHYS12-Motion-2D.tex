...
\end{alertblock}
\note{[P0] "A person walks through a city grid"\\\\
...
\note{[P0] "Head-to-tail method shown step by step"\\\\
...
\note{[P0] "Vector subtraction uses addition of negative vector"\\\\
...
\note{[P0] "Two skaters push on a third - force vectors at right angles"\\\\
...
\end{itemize}
\note{[P0] "Trigonometry is the key to analytical method"\\\\
...
\end{figure}
\note{[P0] [THE REVELATION] "Every 2D vector splits into x and y components"\\\\
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.45\textheight,keepaspectratio]{phys12-motion-2d-fig34-1.jpg}
\end{figure}
...
[P1] "Diagram shows cannonball in free fall vs projectile motion"\\\\
- Both fall at same rate vertically\\\\
...
\note{[P0] "Ball kicked at angle follows parabolic trajectory"\\\\
