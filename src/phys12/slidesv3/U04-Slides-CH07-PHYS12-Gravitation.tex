...
\end{figure}
\note{[Fig 7.1: Historical portraits of Kepler and Newton] "Use visual to establish historical context - these aren't just equations, real people made these discoveries"\\\\
...
\note{[Fig 7.2: Three ellipses showing orbital geometry with aphelion/perihelion labeled] "Point to labeled features - students need to connect vocabulary to visual"\\\\
[P0] "First law: orbits are ellipses, not circles"\\\\
...
\note{[Fig 7.3: Hand with pen drawing ellipse using two pins and string] "Kinesthetic learning - students should actually try this with pins and string in lab"\\\\
[P0] "Ellipse has special property"\\\\
...
\textbf{If you know perihelion $\disp{r_p}$ and aphelion $\disp{r_a}$:}

\pause
\begin{align}
\text{Semi-major axis: } \disp{a} &= \frac{\disp{r_a} + \disp{r_p}}{2} \\
\text{Semi-minor axis: } \disp{b} &= \sqrt{\disp{r_a} \disp{r_p}}
\end{align}
...
\end{alertblock}
\note{[Fig 7.4: Orbital diagram with three shaded sectors A1, A2, A3 of equal area] "Trace finger along orbit - show how distance changes but time stays equal"\\\\
[P0] "Second law: equal areas in equal times"\\\\
...
\begin{center}
\Large
$$\boxed{\frac{\tvar{T_1}^2}{\tvar{T_2}^2} = \frac{\disp{r_1}^3}{\disp{r_2}^3}}$$
\end{center}
...
\textbf{Where:}
\begin{itemize}
\item $\tvar{T}$ = period (\tvar{time} for one orbit)
\item $\disp{r}$ = average orbital \disp{radius}
\end{itemize}
...
\textbf{Given:}
\begin{itemize}
\item Moon: $\tvar{T_1} = 27.3$ d, $\disp{r_1} = 3.84 \times 10^8$ m
\item Satellite: altitude = 1500 km above surface
\item Earth \disp{radius} = 6380 km
\end{itemize}
...
\begin{enumerate}
\item What is $\disp{r_2}$ for satellite? Did you add Earth's \disp{radius}?
...
\textbf{Step 1:} Find orbital \disp{radius}: $\disp{r_2} = 1500 \text{ km} + 6380 \text{ km} = 7880 \text{ km}$

\pause
\vspace{0.2cm}

\textbf{Step 2:} Use Kepler's third law: $\tvar{T_2} = \tvar{T_1} \left(\frac{\disp{r_2}}{\disp{r_1}}\right)^{3/2}$

\pause
\vspace{0.2cm}

\textbf{Step 3:} Substitute and solve:
$$\tvar{T_2} = (27.3 \text{ d}) \left(\frac{24.0 \text{ h}}{\text{d}}\right) \left(\frac{7880 \text{ km}}{3.84 \times 10^5 \text{ km}}\right)^{3/2}$$

\pause
$$\boxed{\tvar{T_2} = 1.93 \text{ h}}$$
...
\note{[Fig 7.7: Painting of Newton under apple tree with falling apple] "Use iconic image to make abstract force concrete - students remember stories"\\\\
[P0] "Legendary apple story has truth"\\\\
...
\begin{center}
\Large
$$\boxed{\force{F} = \grav{G}\frac{\mass{m}\mass{M}}{\disp{r}^2}}$$
\end{center}
Every \mass{mass} attracts every other \mass{mass} with \force{force} proportional to product of \mass{masses} and inversely proportional to \disp{distance} squared.
\end{block}
...
\textbf{Where:}
\begin{itemize}
\item $\force{F}$ = gravitational \force{force} (N)
\item $\grav{G} = 6.67 \times 10^{-11}$ N·m²/kg² (universal constant)
\item $\mass{m}, \mass{M}$ = \mass{masses} (kg)
\item $\disp{r}$ = \disp{distance} between centers (m)
\end{itemize}
...
\textbf{Result:} $\grav{G} = 6.67 \times 10^{-11}$ N·m²/kg²

\pause
Cavendish's value differs less than 1\% from modern value!
\note{[Fig 7.9: Cavendish torsion balance apparatus with spheres and laser] "Show precision needed - emphasize experimental physics requires clever design"\\\\
[P0] "G measured by Henry Cavendish 1798"\\\\
...
\textbf{\mass{Mass}:} Amount of matter (kg) - constant everywhere

\pause
\textbf{\force{Weight}:} Gravitational \force{force} (N) - varies by location
$$\force{W} = \mass{m}\grav{g}$$
...
\textbf{Result:}
$$\frac{\disp{r}^3}{\tvar{T}^2} = \frac{\grav{G}\mass{M}}{4\pi^2}$$

\pause
\vspace{0.3cm}

\begin{exampleblock}{The Revelation}
Kepler said WHAT (pattern). Newton said WHY (gravitational \force{force} causes it).
\end{exampleblock}

\pause
\textbf{Power:} If you know $\disp{r}$ and $\tvar{T}$, can calculate \mass{mass} $\mass{M}$ of parent body!
...
\begin{exampleblock}{The Challenge (3 min, silent)}
Value of $\grav{g}$ on Mars is 3.71 m/s². You have \mass{mass} 60.0 kg on Earth.

\vspace{0.3cm}

\textbf{Given:}
\begin{itemize}
\item Your \mass{mass}: $\mass{m} = 60.0$ kg
\item Mars gravity: $\grav{g_M} = 3.71$ m/s²
\item Earth gravity: $\grav{g_E} = 9.80$ m/s²
\end{itemize}
...
\begin{enumerate}
\item Does \mass{mass} change when go Mars? Why or why not?
\item What formula use for \force{weight} on Mars?
\item How Mars \force{weight} compare Earth \force{weight}?
\end{enumerate}
...
\textbf{Part 1:} \mass{Mass} on Mars = \textbf{60.0 kg} (\mass{mass} constant!)

\pause
\vspace{0.3cm}

\textbf{Part 2:} \force{Weight} on Mars: $\force{W_M} = \mass{m}\grav{g_M}$

\pause
$$\force{W_M} = (60.0 \text{ kg})(3.71 \text{ m/s}^2)$$

\pause
$$\boxed{\force{W_M} = 223 \text{ N}}$$

\pause
\vspace{0.3cm}

\textbf{Compare:} Earth \force{weight} = $(60.0)(9.80) = 588$ N
...
\note{[Fig 7.11: Diagram showing light path bending from distant stars past sun to Earth observer] "Trace light path - bent vs straight lines show spacetime curvature visually"\\\\
[P0] "Einstein's first major test: 1919 solar eclipse"\\\\
...
\textbf{Given:}
\begin{itemize}
\item $\grav{G} = 6.67 \times 10^{-11}$ N·m²/kg²
\item Earth \mass{mass}: $\mass{M} = 5.98 \times 10^{24}$ kg
\item Earth-moon \disp{distance}: $\disp{r} = 3.84 \times 10^8$ m
\end{itemize}

\textbf{Find:} Value $\grav{g}$ at moon's \disp{distance} from Earth
...
\begin{enumerate}
\item What formula use for $\grav{g}$?
\item How handle scientific notation?
\item How this $\grav{g}$ compare Earth's surface $\grav{g} = 9.8$ m/s²?
\end{enumerate}
...
\textbf{Formula:} $\grav{g} = \grav{G}\frac{\mass{M}}{\disp{r}^2}$

\pause
\vspace{0.2cm}

\textbf{Substitute:}
$$\grav{g} = \left(6.67 \times 10^{-11} \frac{\text{N·m}^2}{\text{kg}^2}\right) \left(\frac{5.98 \times 10^{24} \text{ kg}}{(3.84 \times 10^8 \text{ m})^2}\right)$$

\pause
\vspace{0.2cm}

$$\grav{g} = \frac{3.99 \times 10^{14}}{1.47 \times 10^{17}}$$

\pause
$$\boxed{\grav{g} = 2.70 \times 10^{-3} \text{ m/s}^2}$$
...
\item Newton discovered gravitational \force{force} explains Kepler's laws \pause
\item Universal gravitation: $\force{F} = \grav{G}\mass{m_1}\mass{m_2}/\disp{r}^2$ \pause
\item \mass{Mass} constant, \force{weight} varies with $\grav{g}$ \pause
...
\begin{align}
\text{Kepler's Third Law: } \frac{\tvar{T_1}^2}{\tvar{T_2}^2} &= \frac{\disp{r_1}^3}{\disp{r_2}^3} \\
\text{Universal Gravitation: } \force{F} &= \grav{G}\frac{\mass{m}\mass{M}}{\disp{r}^2} \\
\text{Gravitational Constant: } \grav{G} &= 6.67 \times 10^{-11} \text{ N·m}^2/\text{kg}^2 \\
\text{\force{Weight}: } \force{W} &= \mass{m}\grav{g} \\
\text{Surface Gravity: } \grav{g} &= \grav{G}\frac{\mass{M}}{\disp{r}^2} \\
\text{Orbital Constant: } \frac{\disp{r}^3}{\tvar{T}^2} &= \frac{\grav{G}\mass{M}}{4\pi^2}
\end{align}
