...
\begin{itemize}
\item Reduce \force{force} required \pause
\item Increase \disp{distance} over which \force{force} acts \pause
\item Product $\force{f} \times \disp{d}$ stays constant
\end{itemize}

\pause
\begin{alertblock}{The Illusion}
Machines make \work{work} "easier" but don't reduce total \work{work}.
\end{alertblock}
...
\begin{center}
\Large $\boxed{\work{W_i} = \work{W_o}}$
\end{center}
\work{Work} input equals \work{work} output (in ideal case)
\end{block}
...
Expanding this:
$$\force{F_e} \times \disp{d_e} = \force{F_r} \times \disp{d_r}$$

\pause
\vspace{0.2cm}

\begin{exampleblock}{The Mental Model}
If effort \force{force} $\force{F_e}$ is less than resistance \force{force} $\force{F_r}$,\\
then effort \disp{distance} $\disp{d_e}$ must be greater than resistance \disp{distance} $\disp{d_r}$.
\end{exampleblock}
...
\textbf{For a lever:}
$$IMA = \frac{\disp{L_e}}{\disp{L_r}}$$

\pause
\vspace{0.2cm}

\textbf{General formula:}
$$IMA = \frac{\force{F_r}}{\force{F_e}} = \frac{\disp{d_e}}{\disp{d_r}}$$
...
\pause
\textbf{IMA formula:}
$$IMA = \frac{\radius{r_{wheel}}}{\radius{r_{axle}}}$$
...
\pause
\textbf{IMA formula:}
$$IMA = \frac{\disp{L}}{\disp{h}}$$

where $\disp{L}$ is length of ramp and $\disp{h}$ is height
...
\textbf{IMA formula:}
$$IMA = \frac{2\pi \disp{L}}{P}$$

where $\disp{L}$ is handle length and $P$ is pitch (\disp{distance} between threads)
...
\textbf{Ideal machine:} $\work{W_i} = \work{W_o}$ (100\% efficiency)\\
\textbf{Real machine:} $\work{W_o} < \work{W_i}$ (always less than 100\%)
\end{alertblock}
...
$$\text{Efficiency} = \frac{\work{W_o}}{\work{W_i}} \times 100\%$$

\pause
\vspace{0.3cm}

Since $\work{W} = \force{F} \times \disp{d}$:
$$\text{Efficiency} = \frac{\force{F_o} \times \disp{d_o}}{\force{F_i} \times \disp{d_i}} \times 100\%$$
...
\textbf{Given:}
\begin{itemize}
\item $\force{F_i} = 11$ N, $\disp{d_i} = 0.4$ m
\item $\force{F_o} = 40$ N, $\disp{d_o} = 0.1$ m
\end{itemize}
...
\item Did you calculate $\work{W_i}$ and $\work{W_o}$ separately?
...
\textbf{Step 1:} Calculate input \work{work}
$$\work{W_i} = \force{F_i} \times \disp{d_i} = 11 \times 0.4 = 4.4 \text{ J}$$

\pause
\textbf{Step 2:} Calculate output \work{work}
$$\work{W_o} = \force{F_o} \times \disp{d_o} = 40 \times 0.1 = 4.0 \text{ J}$$

\pause
\textbf{Step 3:} Calculate efficiency
$$\text{Efficiency} = \frac{\work{W_o}}{\work{W_i}} \times 100\% = \frac{4.0}{4.4} \times 100\% = \boxed{91\%}$$
...
\textbf{Given:}
\begin{itemize}
\item Length $\disp{L} = 5.0$ m
\item Height $\disp{h} = 2.0$ m
\end{itemize}
...
\textbf{Interpretation:} This ramp multiplies your \force{force} by 2.5.\\
You can push a 250 N load with only 100 N effort.\\
The trade-off: you push 2.5 times the \disp{distance}.
...
\textbf{Given:}
\begin{itemize}
\item $\force{F_r} = 200$ N (load)
\item $\force{F_e} = 52$ N (effort)
...
$$MA = \frac{\force{F_r}}{\force{F_e}} = \frac{200 \text{ N}}{52 \text{ N}} = 3.85$$
...
\begin{enumerate}
\item \textbf{Lever:} $IMA = \frac{\disp{L_e}}{\disp{L_r}}$ \pause
\item \textbf{Wheel and Axle:} $IMA = \frac{\radius{r_{wheel}}}{\radius{r_{axle}}}$ \pause
\item \textbf{Inclined Plane:} $IMA = \frac{\disp{L}}{\disp{h}}$ \pause
\item \textbf{Wedge:} $IMA = \frac{\disp{L}}{\disp{h}}$ \pause
\item \textbf{Screw:} $IMA = \frac{2\pi \disp{L}}{P}$ \pause
...
\begin{align}
IMA &= \frac{\force{F_r}}{\force{F_e}} = \frac{\disp{d_e}}{\disp{d_r}} \\
\work{W_i} &= \work{W_o} \quad \text{(ideal machine)} \\
\force{F_e} \times \disp{d_e} &= \force{F_r} \times \disp{d_r} \\
\text{Efficiency} &= \frac{\work{W_o}}{\work{W_i}} \times 100\%
\end{align}
...
\begin{enumerate}
\item Simple machines trade \force{force} for \disp{distance} \pause
...
\item IMA tells you how much \force{force} is multiplied \pause
\item Efficiency measures \work{work} lost to friction \pause
