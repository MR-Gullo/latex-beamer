...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth,height=0.6\textheight,keepaspectratio]{phys12-relativity-fig01.jpg}
...
\end{alertblock}
\note{[P0] "The Orion Nebula - thousands of light years away"\\\\
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys12-relativity-fig02.jpg}
...
\note{[P0] "Michelson and Morley built an interferometer in 1887"\\\\
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth,height=0.4\textheight,keepaspectratio]{phys12-relativity-fig03.jpg}
...
\note{[P0] "Albert Einstein - 1879 to 1955"\\\\
...
\end{center}
The speed of light is the same in all inertial frames and is NOT affected by the speed of its source.
\end{block}
...
\begin{center}
\Large $c = 299{,}792{,}458 \text{ m/s}$
\end{center}

\pause
\vspace{0.3cm}

For most purposes: $c \approx 3.00 \times 10^8 \text{ m/s}$
...
\textbf{Physicist:} "Light always travels at $c$, regardless of source speed."
\end{alertblock}
...
\textbf{Your brain says:} Light speed = $c + 200$ m/s\\
\textbf{Reality:} Light speed = $c$ (always)
\end{alertblock}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth,height=0.5\textheight,keepaspectratio]{phys12-relativity-fig04.jpg}
...
\note{[P0] "Observer A sits in center of moving train - two flash lamps fire"\\\\
...
\textbf{Key insight:} $t = \frac{d}{v}$

\pause
\vspace{0.3cm}

If light speed $c$ is constant for all observers...

\pause
\vspace{0.3cm}

...and distance $d$ appears different to different observers...

\pause
\vspace{0.3cm}

\alert{...then time $t$ must also be different!}
...
\textbf{Given:}
\begin{itemize}
\item Distance $d = 1.50 \times 10^8$ km
...
\textbf{Equation:} $v = \frac{d}{t}$ so $t = \frac{d}{v}$
...
\textbf{Substitute:} $t = \frac{1.50 \times 10^{11} \text{ m}}{3.00 \times 10^8 \text{ m/s}}$

\pause
\vspace{0.2cm}

$$\boxed{t = 500 \text{ s} = 8 \text{ min } 20 \text{ s}}$$
...
\begin{center}
\Large $\boxed{\gamma = \frac{1}{\sqrt{1 - \frac{v^2}{c^2}}}}$
\end{center}
...
\textbf{When $v \ll c$:} $\gamma \approx 1$ (classical physics works)

\pause
\textbf{When $v \approx c$:} $\gamma \gg 1$ (relativistic effects dominate)
...
\textbf{Try some values for $v$:}

\pause
\vspace{0.3cm}

\begin{itemize}
\item Airliner: $v = 225$ m/s $\rightarrow$ $\gamma = 1.0000000003$ \pause
\item Earth's orbit: $v = 2.98 \times 10^4$ m/s $\rightarrow$ $\gamma = 1.000000005$ \pause
\item Particle accelerator: $v = 2.99 \times 10^8$ m/s $\rightarrow$ $\gamma = 7.1$
\end{itemize}
...
\begin{center}
\Large $\boxed{\Delta t = \gamma \Delta t_0}$
\end{center}
Time passes MORE SLOWLY for an observer moving relative to you.
\end{block}
...
\begin{itemize}
\item $\Delta t_0$ = proper time (measured by moving observer) \pause
\item $\Delta t$ = dilated time (measured by stationary observer) \pause
\item $\Delta t > \Delta t_0$ always
\end{itemize}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth,height=0.5\textheight,keepaspectratio]{phys12-relativity-fig05.jpg}
\caption{Light crossing a moving spacecraft}
\end{figure}

\pause
\textbf{Astronaut measures:} Time $\Delta t_0$ (shorter path)

\pause
\textbf{Earth observer measures:} Time $\Delta t$ (longer path)

\pause
\alert{Same light, different distances, different times!}
\note{[P0] "Astronaut measures light crossing her ship - vertical path"\\\\
...
\begin{center}
\Large $\boxed{L = \frac{L_0}{\gamma} = L_0\sqrt{1 - \frac{v^2}{c^2}}}$
\end{center}
...
\begin{itemize}
\item $L_0$ = proper length (measured at rest) \pause
\item $L$ = contracted length (measured by moving observer) \pause
\item $L < L_0$ always
\end{itemize}
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys12-relativity-fig06.jpg}
...
\pause
\alert{Because $v = \frac{d}{t}$ and you agree on $v$ but not on $t$, you must also disagree on $d$!}
\note{[P0] "At everyday speeds: you both measure same distance"\\\\
...
\textbf{Given:}
\begin{itemize}
\item Proper length $L_0 = 50$ m
\item Velocity $v = 0.95c$
\end{itemize}

\textbf{Find:} Contracted length $L$
...
\textbf{Equation:} $L = L_0\sqrt{1 - \frac{v^2}{c^2}}$

\pause
\vspace{0.2cm}

\textbf{Substitute:} $L = 50 \text{ m}\sqrt{1 - \frac{(0.95c)^2}{c^2}}$

\pause
\vspace{0.2cm}

\textbf{Simplify:} $L = 50 \text{ m}\sqrt{1 - (0.95)^2}$

\pause
\vspace{0.2cm}

$$L = 50 \text{ m}\sqrt{1 - 0.9025} = 50 \text{ m}\sqrt{0.0975}$$
...
\begin{center}
\Large $\boxed{p = \gamma mu}$
\end{center}
Momentum increases without limit as velocity approaches $c$.
\end{block}
...
\begin{itemize}
\item $m$ = rest mass \pause
\item $u$ = velocity of object \pause
\item As $u \rightarrow c$, $\gamma \rightarrow \infty$, so $p \rightarrow \infty$
\end{itemize}
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys12-relativity-fig07.jpg}
\caption{Relativistic momentum approaches infinity}
\end{figure}

\pause
\alert{As $v \rightarrow c$, momentum $p \rightarrow \infty$}

\pause
This is why you can't reach the speed of light!
\note{[P0] "Graph shows relativistic momentum vs velocity"\\\\
...
\Huge $\boxed{E = mc^2}$
\end{center}
Mass and energy are interchangeable. Matter IS energy.
\end{block}
...
\begin{itemize}
\item $E$ = rest energy (joules) \pause
\item $m$ = rest mass (kg) \pause
...
\begin{frame}
\frametitle{10.2 The Power of $c^2$}
\textbf{How much energy in 1 gram of matter?}

\pause
\vspace{0.3cm}

$E = mc^2 = (0.001 \text{ kg})(3.00 \times 10^8 \text{ m/s})^2$
...
Converting 1 gram of mass to energy: $9.0 \times 10^{13}$ J
\end{exampleblock}
...
\begin{center}
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{phys12-relativity-fig08.jpg}
...
\column{0.48\textwidth}
\begin{center}
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{phys12-relativity-fig09.jpg}
...
\note{[P0] "The Sun - fusion converts hydrogen to helium"\\\\
...
\alert{Mass defect: 0.0327 u}

\pause
\vspace{0.3cm}

This "missing" mass became binding energy when the nucleus formed:

$E = (5.04 \times 10^{-30} \text{ kg})(3.00 \times 10^8 \text{ m/s})^2 = 4.54 \times 10^{-12}$ J
...
\textbf{Given:}
\begin{itemize}
\item Both particles have rest mass $m = 9.11 \times 10^{-31}$ kg
\item Total mass: $2 \times 9.11 \times 10^{-31}$ kg
\end{itemize}

\textbf{Find:} Energy $E$ in joules
...
\textbf{Equation:} $E = mc^2$

\pause
\vspace{0.2cm}

\textbf{Total mass:} $m = 2(9.11 \times 10^{-31} \text{ kg}) = 1.822 \times 10^{-30} \text{ kg}$

\pause
\vspace{0.2cm}

\textbf{Substitute:} $E = (1.822 \times 10^{-30} \text{ kg})(3.00 \times 10^8 \text{ m/s})^2$

\pause
\vspace{0.2cm}

$$E = (1.822 \times 10^{-30})(9.00 \times 10^{16})$$

\pause
$$\boxed{E = 1.64 \times 10^{-13} \text{ J}}$$
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys12-relativity-fig10.jpg}
...
\pause
\textbf{Result:} Time dilates by factor of 13, length contracts by factor of 13

\pause
\textbf{Goal:} Recreate conditions from the Big Bang!
\note{[P0] "RHIC: Relativistic Heavy Ion Collider at Brookhaven"\\\\
...
\begin{align*}
\text{Time Dilation:} \quad & \Delta t = \gamma \Delta t_0 \\
\text{Length Contraction:} \quad & L = \frac{L_0}{\gamma} \\
...
\textbf{Where:} $\gamma = \frac{1}{\sqrt{1 - \frac{v^2}{c^2}}}$
...
\item Two postulates: physics is universal, $c$ is constant \pause
...
\item $E = mc^2$: mass and energy are equivalent
\end{enumerate}
\end{block}
...
\item Momentum $\rightarrow \infty$ \pause
\item Energy required $\rightarrow \infty$
\end{itemize}
