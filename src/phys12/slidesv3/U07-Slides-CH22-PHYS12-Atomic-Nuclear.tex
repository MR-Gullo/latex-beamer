...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.55\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-2.jpg}
\end{figure}
...
\end{columns}
\note{[Fig 22.2: Rutherford setup - alpha source, gold foil, screen] "Show how detection screen surrounds foil 360° - this catches backward deflections that shocked Rutherford"\\\\
[P0] [THE HOOK] "Imagine firing bullets at tissue paper and having them bounce back"\\\\
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.45\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-3.jpg}
\end{figure}
...
\end{itemize}
\end{columns}
\note{[Fig 22.3: Gold atoms showing tiny nuclei] "Point to size comparison: nucleus 10⁻¹⁵ m vs atom 10⁻¹⁰ m - that's 100,000× difference. Use analogy: if nucleus = marble, electron cloud = football stadium"\\\\
[P0] [THE REVELATION] "Most of the atom is vacuum"\\\\
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth,height=0.5\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-4.jpg}
\end{figure}
...
\end{alertblock}
\note{[Fig 22.4: Atom with orbital electrons] "Trace circular orbits with finger - emphasize centripetal acceleration. Ask: 'What happens to accelerating charges?' They radiate! This creates the paradox"\\\\
[P0] "Rutherford proposed planetary model"\\\\
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.4\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-5.jpg}
\end{figure}
...
\end{block}
\note{[Fig 22.5: Emission spectrum setup and result] "Point to part (a) equipment: hot gas → light → grating → discrete lines. Part (b) shows actual iron spectrum. Emphasize: NOT continuous like a rainbow"\\\\
[P0] "Spectroscopy: the quantum detective work"\\\\
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth,height=0.4\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-6.jpg}
\end{figure}
...
But why?
\end{exampleblock}
\note{[Fig 22.6: Hydrogen spectrum series] "Show three regions: Lyman 91-122nm (UV), Balmer 365-656nm (visible - students can see these!), Paschen 820-1875nm (IR). Point out lines get closer at series limit"\\\\
[P0] "Hydrogen: simplest atom, richest spectrum"\\\\
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.45\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-9.jpg}
\end{figure}
...
\Large
$$\energy{E_n} = -\frac{13.6 \text{ eV}}{\particles{n}^2} \quad (\particles{n}=1,2,3,\ldots)$$
\end{block}
\note{[Fig 22.9: Bohr model with quantized orbits] "Trace energy levels increasing outward. Show photon absorption (arrow up) vs emission (arrow down). Emphasize: electron doesn't spiral - it JUMPS"\\\\
[P0] [THE REVELATION] "Bohr said: orbits are quantized"\\\\
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth,height=0.6\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-11.jpg}
\end{figure}
...
\end{itemize}
\end{exampleblock}
\note{[Fig 22.11: Energy level diagram with series] "Show Lyman (to n=1), Balmer (to n=2), Paschen (to n=3) arrows. Point out: longer arrow = more energy = shorter wavelength. Connect back to Fig 22.6 spectrum"\\\\
[P0] "Vertical axis: energy"\\\\
...
\textbf{Given:}
\begin{itemize}
\item Initial state: $\particles{n_i} = 1$
\item Final state: $\particles{n_f} = 3$
\item $\energy{E_n} = -\frac{13.6 \text{ eV}}{\particles{n}^2}$
\end{itemize}
...
\textbf{U - Unknown:} $\Delta \energy{E} = ?$

\pause
\textbf{E - Equations:}
$$\energy{E_n} = -\frac{13.6 \text{ eV}}{\particles{n}^2}, \quad \Delta \energy{E} = \energy{E_f} - \energy{E_i}$$

\pause
\textbf{S - Substitute:}
$$\energy{E_i} = -\frac{13.6}{1^2} = -13.6 \text{ eV}$$
$$\energy{E_f} = -\frac{13.6}{3^2} = -1.51 \text{ eV}$$

\pause
$$\Delta \energy{E} = (-1.51) - (-13.6) = +12.09 \text{ eV}$$

\pause
$$\boxed{\Delta \energy{E} = 12.1 \text{ eV}}$$
...
\Large
$$\energy{E} = \pConst{h}\freq{f} = \frac{\pConst{h}\vel{c}}{\wavelen{\lambda}}$$
\end{block}

\pause
\textbf{Where:}
\begin{itemize}
\item $\pConst{h} = 6.626 \times 10^{-34}$ J$\cdot$s (Planck's constant)
\item $\vel{c} = 3.00 \times 10^8$ m/s (speed of light)
\item $\freq{f}$ = frequency (Hz)
\item $\wavelen{\lambda}$ = wavelength (m)
\end{itemize}

\pause
\begin{exampleblock}{The Connection}
Higher \energy{energy} transition $\rightarrow$ shorter \wavelen{wavelength} photon
\end{exampleblock}
...
\Large
$$\frac{1}{\wavelen{\lambda}} = \pConst{R}\left(\frac{1}{\particles{n_f}^2} - \frac{1}{\particles{n_i}^2}\right)$$
\end{block}

\textbf{Where:}
\begin{itemize}
\item $\pConst{R} = 1.097 \times 10^7$ m$^{-1}$ (Rydberg constant)
\item $\particles{n_i}$ = initial quantum number (higher)
\item $\particles{n_f}$ = final quantum number (lower)
\end{itemize}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-12.jpg}
\end{figure}
...
\pause
\textbf{Yes!}
$$\wavelen{\lambda} = \frac{\pConst{h}}{\mom{p}}$$

Only certain \wavelen{wavelengths} fit in orbit $\rightarrow$ quantization!
\end{columns}
\note{[Fig 22.12: Standing waves - allowed vs forbidden] "Show part (a) standing wave on string. Parts (b)/(c): allowed orbit has integer wavelengths (2πr=nλ constructive), forbidden doesn't (destructive). This explains quantization!"\\\\
[P0] [THE HOOK] "Matter waves: the ultimate twist"\\\\
...
\Large
$$\Delta \disp{x} \Delta \mom{p} \geq \frac{\pConst{h}}{4\pi}$$
\end{block}
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth,height=0.5\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-15.jpg}
\end{figure}
...
\end{itemize}
\end{block}
\note{[Fig 22.15: 3D probability cloud] "Show spherical coordinate system with nucleus at origin. The sphere indicates most probable distance for electron - NOT an orbit! Ask: Can you say exactly where electron is? No!"\\\\
[P0] "Modern view: electron cloud"\\\\
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-16.jpg}
\end{figure}
...
\pause
\textbf{Notation:}
$$^{\particles{A}}_{\particles{Z}} X_{\particles{N}}$$
\begin{itemize}
\item $\particles{Z}$ = atomic number (protons)
\item $\particles{A}$ = mass number (protons + neutrons)
\item $\particles{N} = \particles{A} - \particles{Z}$ (neutrons)
\end{itemize}
\end{columns}
\note{[Fig 22.16: Lithium-7 atom structure] "Point to nucleus: 3 protons (red) + 4 neutrons (gray). Count them! Then show 3 electrons orbiting. Emphasize: protons determine element identity"\\\\
[P0] "Nucleus: protons and neutrons"\\\\
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-17.jpg}
\end{figure}
\end{columns}
\note{[Fig 22.17: Nucleus with nucleons held together] "Show how protons (positive) would fly apart from Coulomb repulsion. Strong force (attractive between ALL nucleons) overcomes this but only at ~10⁻¹⁵ m range"\\\\
[P0] [THE CONFLICT] "Protons hate each other"\\\\
...
Einstein's $\energy{E} = \mass{m}\vel{c}^2$ explains it:\\
\mass{Mass} converts to \energy{energy} in nuclear reactions
\end{exampleblock}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-18.jpg}
\end{figure}

\column{0.5\textwidth}
\begin{block}{Nuclear Equation}
$$^{\particles{A}}_{\particles{Z}} X_{\particles{N}} \rightarrow ^{\particles{A}-4}_{\particles{Z}-2} Y_{\particles{N}-2} + ^4_2 He_2$$
\end{block}
...
\textbf{Conservation:}
\begin{itemize}
\item \particles{Mass number}: 238 = 234 + 4 \checkmark
\item \charge{Charge}: 92 = 90 + 2 \checkmark
\end{itemize}
\end{columns}
\note{[Fig 22.18: Alpha decay - nucleus releasing He nucleus] "Show parent nucleus before, then alpha (2p+2n) ejected, leaving smaller daughter. Emphasize: alpha = He-4 nucleus, very stable configuration"\\\\
[P0] "Alpha: eject helium nucleus"\\\\
- "Reduces A by 4"\\\\
- "Reduces Z by 2"\\\\
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-19.jpg}
\end{figure}

\column{0.5\textwidth}
\begin{block}{$\beta^-$ Decay (most common)}
Neutron $\rightarrow$ proton + electron + antineutrino
$$^{\particles{A}}_{\particles{Z}} X_{\particles{N}} \rightarrow ^{\particles{A}}_{\particles{Z}+1} Y_{\particles{N}-1} + e^- + \bar{\nu}_e$$
\end{block}
...
\end{columns}
\note{[Fig 22.19: Beta decay - neutron splitting] "Show neutron transforming into proton (stays in nucleus) + electron (ejected) + antineutrino (invisible, escapes). Three products from one particle!"\\\\
[P0] "Beta: neutron transforms"\\\\
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-20.jpg}
\end{figure}

\column{0.5\textwidth}
\begin{block}{Excited Nucleus}
Nucleus drops from excited state to ground state
$$^{\particles{A}}_{\particles{Z}} X^* \rightarrow ^{\particles{A}}_{\particles{Z}} X + \gamma$$
\end{block}

\pause
\textbf{Key Points:}
\begin{itemize}
\item $\particles{A}$ and $\particles{Z}$ unchanged
\item Pure \energy{energy} release
\item Often follows $\alpha$ or $\beta$ decay
\item MeV \energy{energies} (vs. eV for atoms)
\end{itemize}
\end{columns}
\note{[Fig 22.20: Gamma ray emission from nucleus] "Show excited nucleus (marked *) releasing wavy gamma ray. Nucleus identical before/after - only energy state changes. Compare to electron transitions but MeV vs eV scale"\\\\
[P0] "Gamma: nucleus de-excites"\\\\
...
\textbf{General form:}
$$^{\particles{A}}_{\particles{Z}} X \rightarrow ^{\particles{A}-4}_{\particles{Z}-2} Y + ^4_2 He$$

\pause
\textbf{Apply to Pu-239:}
$$\particles{A}_{daughter} = 239 - 4 = 235$$
$$\particles{Z}_{daughter} = 94 - 2 = 92$$

\pause
\textbf{Identify element:} $\particles{Z} = 92$ is Uranium (U)
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-24.jpg}
\end{figure}

\begin{block}{Definition}
\textbf{Half-life ($\tvar{t_{1/2}}$):} Time for half the nuclei to decay
\end{block}

\pause
\begin{itemize}
\item After $\tvar{t_{1/2}}$: $\particles{N} \rightarrow \particles{N}/2$
\item After $2\tvar{t_{1/2}}$: $\particles{N} \rightarrow \particles{N}/4$
\item After $3\tvar{t_{1/2}}$: $\particles{N} \rightarrow \particles{N}/8$
\end{itemize}
\note{[Fig 22.24: Exponential decay graph with table] "Trace curve: starts 1000, drops to 500 (1 half-life), 250 (2), 125 (3). Show table reinforces pattern. After 10 half-lives: less than 0.1% remains"\\\\
[P0] [THE HOOK] "Exponential decay: nature's countdown"\\\\
...
\Large
$$\particles{N}(\tvar{t}) = \particles{N_0} e^{-\lambda \tvar{t}}$$
\end{block}

\textbf{Where:}
\begin{itemize}
\item $\particles{N_0}$ = initial number of nuclei
\item $\particles{N}(\tvar{t})$ = number remaining at time $\tvar{t}$
...
$$\lambda = \frac{\ln(2)}{\tvar{t_{1/2}}} \approx \frac{0.693}{\tvar{t_{1/2}}}$$
\end{block}
...
$$R = \frac{\Delta \particles{N}}{\Delta \tvar{t}} = \lambda \particles{N}$$
\end{block}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-25.jpg}
\end{figure}
...
\end{columns}
\note{[Fig 22.25: Shroud of Turin photo and negative] "Show actual shroud and its photographic negative revealing image detail. This sparked authenticity debate for centuries until C-14 dating settled it"\\\\
[P0] "Famous controversy"\\\\
- "Appeared in 1354 CE"\\\\
...
\textbf{Given:}
\begin{itemize}
\item $\particles{N_0} = 80$ g
\item $\tvar{t_{1/2}} = 10$ days
\item Time elapsed: $\tvar{t} = 30$ days
\end{itemize}
...
Number of half-lives: $\particles{n} = \frac{\tvar{t}}{\tvar{t_{1/2}}} = \frac{30}{10} = 3$

\pause
After each half-life:
\begin{itemize}
\item $\tvar{t} = 10$ days: $80 \div 2 = 40$ g
\item $\tvar{t} = 20$ days: $40 \div 2 = 20$ g
\item $\tvar{t} = 30$ days: $20 \div 2 = 10$ g
\end{itemize}

\pause
\textbf{Method 2 - Formula:}
$$\particles{N} = \particles{N_0} \left(\frac{1}{2}\right)^{\particles{n}} = 80 \left(\frac{1}{2}\right)^3 = 80 \times \frac{1}{8} = 10 \text{ g}$$
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-29.jpg}
\end{figure}
...
\end{itemize}
\note{[Fig 22.29: Binding energy per nucleon curve] "Trace curve: rises from H (low), peaks at Fe-56 (~9 MeV/nucleon), slowly falls for heavier. Moving UP releases energy. Point to fusion region (left of Fe), fission region (right of Fe)"\\\\
[P0] [THE REVELATION] "This curve explains everything"\\\\
- "Why stars shine"\\\\
- "Why bombs explode"\\\\
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-26.jpg}
\end{figure}
...
\note{[Fig 22.26: Fission process stages] "Show (a) neutron approaches, (b) nucleus deforms like water droplet, (c) splits into two fragments, (d) releases 2-3 neutrons. Emphasize stage (d): those neutrons can cause more fissions!"\\\\
[P0] [THE HOOK] "Splitting the unsplittable"\\\\
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth,height=0.5\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-27.jpg}
\end{figure}
...
\end{alertblock}
\note{[Fig 22.27: Chain reaction diagram] "Trace cascade: 1 fission → 2 neutrons → 2 fissions → 4 neutrons → 4 fissions. Show exponential branching. Each generation takes ~10⁻⁸ seconds!"\\\\
[P0] "Chain reaction: nuclear avalanche"\\\\
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.55\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-28.jpg}
\end{figure}
...
\end{itemize}
\note{[Fig 22.28: Pressurized water reactor schematic] "Trace flow: reactor core heats primary loop → steam generator → turbine → condenser. Point to control rods (inserted to slow/stop reaction). Containment building prevents radiation release"\\\\
[P0] "Controlled chain reaction"\\\\
...
\textbf{In fission:}
\begin{itemize}
\item Products have less \mass{mass} than reactants
\item Missing \mass{mass} $\rightarrow$ converted to \energy{energy}
\item $\Delta \mass{m} \approx 0.1\%$ of original \mass{mass}
\item Still liberates enormous \energy{energy}
\end{itemize}

\pause
\begin{exampleblock}{Example}
1 kg of $^{235}$U fully fissioned:\\
$\energy{E} = 8.2 \times 10^{13}$ J $\approx$ 14,000 barrels of oil!
\end{exampleblock}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-30.jpg}
\end{figure}
...
\end{itemize}
\end{columns}
\note{[Fig 22.30: Coulomb barrier potential energy diagram] "Show PE curve: right side = Coulomb repulsion hump, left side = attractive nuclear well. Nuclei must have enough KE to climb hump (tunnel through). That requires millions of Kelvin!"\\\\
[P0] [THE HOOK] "How does the Sun shine?"\\\\
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth,height=0.4\textheight,keepaspectratio]{phys11-atomic-nuclear-fig22-41.jpg}
\end{figure}
...
\end{alertblock}
\note{[Fig 22.41: Tokamak confinement equipment] "Show donut-shaped magnetic containment chamber. Magnetic field coils keep plasma suspended away from walls. If plasma touches wall: instantly cools, fusion stops. Engineering challenge of the century!"\\\\
[P0] [THE HOOK] "Energy of the future - for 70 years"\\\\
...
$$\energy{E_n} = -\frac{13.6 \text{ eV}}{\particles{n}^2}, \quad \Delta \energy{E} = \energy{E_f} - \energy{E_i}, \quad \energy{E} = \pConst{h}\freq{f} = \frac{\pConst{h}\vel{c}}{\wavelen{\lambda}}$$
$$\frac{1}{\wavelen{\lambda}} = \pConst{R}\left(\frac{1}{\particles{n_f}^2} - \frac{1}{\particles{n_i}^2}\right), \quad \Delta \disp{x} \Delta \mom{p} \geq \frac{\pConst{h}}{4\pi}$$
\end{block}
...
$$\particles{N}(\tvar{t}) = \particles{N_0} e^{-\lambda \tvar{t}}, \quad \lambda = \frac{0.693}{\tvar{t_{1/2}}}, \quad R = \lambda \particles{N}$$
\end{block}

\begin{block}{Nuclear Notation}
$$^{\particles{A}}_{\particles{Z}} X_{\particles{N}} \quad (\particles{A} = \particles{Z} + \particles{N})$$
\end{block}
