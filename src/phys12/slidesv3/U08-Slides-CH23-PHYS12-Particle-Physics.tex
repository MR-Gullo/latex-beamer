...
\begin{itemize}
\item Strongest force at short range ($<10^{-15}$ \disp{m})
...
\begin{center}
\Large $\boxed{\energy{E} = \mass{m}\vel{c}^2}$
\end{center}
\energy{Energy} converts to \mass{matter}
\end{block}

\pause
\begin{itemize}
\item Accelerate known particles to high \energy{energy}
\item Collide them with targets or each other
\item Create new particles from \energy{energy}
\item Probe smaller distances = higher \energy{energies} needed
\end{itemize}
...
Stationary target: much \energy{energy} lost to recoil\\
Colliding beams: particles created with near-zero \mom{momentum}
\end{block}
...
\small
\begin{tabular}{lll}
\textbf{Quark} & \textbf{Symbol} & \textbf{\charge{Charge}} \\ \hline
Up & u & +$\frac{2}{3}$\charge{e} \\
Down & d & -$\frac{1}{3}$\charge{e} \\
Charm & c & +$\frac{2}{3}$\charge{e} \\
Strange & s & -$\frac{1}{3}$\charge{e} \\
Top & t & +$\frac{2}{3}$\charge{e} \\
Bottom & b & -$\frac{1}{3}$\charge{e}
\end{tabular}
\end{center}
\end{block}
...
\textbf{Expected:} \charge{Charge} is discrete (multiples of \charge{e})\\
\textbf{Reality:} Quarks have fractional \charge{charge}!
\end{alertblock}
...
\pause
\charge{Charge}: $\frac{2}{3} + \frac{2}{3} - \frac{1}{3} = +1$ \checkmark
...
\pause
Same \mass{mass} as electron, opposite \charge{charge} = antielectron
...
\end{center}
\mass{Matter} converts to \energy{energy}
\end{block}

\pause
\vspace{0.3cm}

Both \mass{mass}-\energy{energy} and \charge{charge} conserved!
...
\textbf{Pattern:} \mass{Mass} increases left to right

\textbf{Trend:} Higher \mass{mass} = less stable = faster decay
...
\textbf{The problem:} Why do W and Z bosons have \mass{mass}, but photons and gluons don't?
...
\textbf{Evidence:} Particle with predicted \mass{mass}, spin, and interactions
...
\textbf{LHC maximum:} 14 TeV = $1.4 \times 10^4$ \energy{GeV}\\
\textbf{GUT \energy{energy}:} $10^{14}$ \energy{GeV}\\
