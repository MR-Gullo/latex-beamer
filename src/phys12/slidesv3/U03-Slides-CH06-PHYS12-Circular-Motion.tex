...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys12-circular-motion-fig01.jpg}
...
\end{exampleblock}
\note{[P0] "Formula 1 cars - wheels spin, car curves"\\\\
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.55\textheight,keepaspectratio]{phys12-circular-motion-fig04.jpg}
...
$$\boxed{\Delta\theta = \frac{\Delta s}{r}}$$
Angle equals arc length divided by radius
\end{block}
...
\end{columns}
\note{[P0] "Points on a CD all rotate through same angle"\\\\
...
\begin{block}{Universal Law: Angular Velocity}
\begin{center}
\Large $\boxed{\omega = \frac{\Delta\theta}{\Delta t}}$
\end{center}
Angular velocity equals change in angle divided by change in time
\end{block}
...
\begin{exampleblock}{The Mental Model}
Points farther from the center move faster linearly, but all points have the same angular velocity.
\end{exampleblock}

\pause

\textbf{Example:} CD spinning - outer edge moves faster than inner part, but both complete one revolution in same time.
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{phys12-circular-motion-fig08.jpg}
\caption{Car tire rolling}
\end{figure}

\pause

Large $\omega$ means large $v$ because $v = r\omega$

\pause

Larger radius tire at same $\omega$ produces greater $v$
\note{[P0] "Tire spinning - how does car move forward?"\\\\
...
\begin{itemize}
\item Radius $r = 1.0$ m
\item Time: 12 to 3 (quarter rotation)
\end{itemize}
...
Full rotation = $2\pi$ rad, so angle = $\frac{1}{4} \times 2\pi = \boxed{\frac{\pi}{2} \text{ rad}}$

\pause
\vspace{0.3cm}

\textbf{Part (b):} Use $\Delta s = r\Delta\theta$

\pause

$$\Delta s = (1.0 \text{ m})\left(\frac{\pi}{2} \text{ rad}\right) = \boxed{1.6 \text{ m}}$$
...
\textbf{Given:}
\begin{itemize}
\item Radius $r = 0.300$ m
\item Tangential velocity $v = 15.0$ m/s
\end{itemize}

\textbf{Find:} Angular velocity $\omega$ of the tire in rad/s
...
\begin{enumerate}
\item What equation connects linear and angular velocity?
\item How did you rearrange it to solve for $\omega$?
...
\textbf{Equation:} $v = r\omega$, so $\omega = \frac{v}{r}$

\pause

\textbf{Substitute:}
$$\omega = \frac{15.0 \text{ m/s}}{0.300 \text{ m}}$$

\pause

$$\boxed{\omega = 50.0 \text{ rad/s}}$$
...
\textbf{Uniform circular motion:} Object travels circular path at constant speed

\pause

\begin{alertblock}{Civilian View vs. Reality}
\textbf{Civilian:} "Constant speed means no acceleration."\\
\textbf{Physicist:} "Velocity is changing direction, so there IS acceleration."
\end{alertblock}

\pause
\vspace{0.3cm}

Acceleration is a change in velocity - magnitude OR direction!
...
\begin{exampleblock}{The Mental Model}
When you turn in a car, you feel pushed outward. But no force pushes you out - your body wants to go straight (Newton's first law) while the car turns.
\end{exampleblock}
...
The real force is \textbf{centripetal} - pulling you inward toward the center!
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth,height=0.45\textheight,keepaspectratio]{phys12-circular-motion-fig18.jpg}
\caption{Velocity changes direction, acceleration points toward center}
\end{figure}

\pause

\begin{block}{Universal Law: Centripetal Acceleration}
$$\boxed{a_c = \frac{v^2}{r}} \quad \text{or} \quad \boxed{a_c = r\omega^2}$$
\end{block}
\note{[P0] "Velocity arrows change direction - acceleration points inward"\\\\
...
\textbf{Proportional to $v^2$:} Doubling speed means 4 times the acceleration!

\pause

\textbf{Example:}
\begin{itemize}
\item Curve at 50 km/h: moderate acceleration
\item Same curve at 100 km/h: \alert{4 times} the acceleration
\end{itemize}

\pause

\begin{alertblock}{The Warning}
This is why speed limits are lower on curves - small speed increase creates huge acceleration increase.
\end{alertblock}
...
For circular motion: $F_{\text{net}} = ma_c$

\pause

\begin{block}{Universal Law: Centripetal Force}
$$\boxed{F_c = m\frac{v^2}{r}} \quad \text{or} \quad \boxed{F_c = mr\omega^2}$$
\end{block}
...
\begin{exampleblock}{The Mental Model}
Centripetal force isn't a new kind of force - it's whatever force points toward the center and causes circular motion.
\end{exampleblock}
...
\item Mass $m = 900$ kg
\item Radius $r = 600$ m
\item Speed $v = 25.0$ m/s
\end{itemize}

\textbf{Find:} Centripetal force required to keep car on curve
...
\begin{enumerate}
\item What equation did you use for centripetal force?
\item Did you remember to square the velocity?
\item What force provides the centripetal force for a car?
\end{enumerate}
...
\textbf{Equation:} $F_c = m\frac{v^2}{r}$

\pause

\textbf{Substitute:}
$$F_c = \frac{(900 \text{ kg})(25.0 \text{ m/s})^2}{600 \text{ m}}$$

\pause

$$F_c = \frac{(900)(625)}{600} = \boxed{938 \text{ N}}$$

\pause

\textbf{Check:} About 940 N - this is the friction force between tires and road. Without it, car slides straight!
\note{[P0] "Self-correct in different color"\\\\
...
\begin{enumerate}
\item What equation did you use for centripetal acceleration?
\item How did you express it as a fraction of $g$?
\item Is the acceleration large or small compared to gravity?
\end{enumerate}
...
$$a_c = \frac{625}{500} = \boxed{1.25 \text{ m/s}^2}$$

\pause

\textbf{Compare to $g$:}
$$\frac{a_c}{g} = \frac{1.25}{9.80} = 0.128 \quad \Rightarrow \quad \boxed{a_c = 0.13g}$$

\pause

\textbf{Revelation:} Gentle highway curve at moderate speed produces about 1/10th the acceleration of gravity!
\note{[P0] "Self-correct in different color"\\\\
...
\begin{block}{Universal Law: Angular Acceleration}
$$\boxed{\alpha = \frac{\Delta\omega}{\Delta t}}$$
...
$$\boxed{a = r\alpha} \quad \text{or} \quad \boxed{\alpha = \frac{a}{r}}$$
\end{block}

\pause
\vspace{0.3cm}

\textbf{Tangential acceleration:} Linear acceleration along the circle's edge

\pause

\begin{exampleblock}{The Mental Model}
Greater angular acceleration means greater tangential acceleration. Points farther from center have larger tangential acceleration for same $\alpha$.
\end{exampleblock}
...
Every linear kinematics equation has a rotational analog. Just swap $x \rightarrow \theta$, $v \rightarrow \omega$, $a \rightarrow \alpha$!
\end{alertblock}
...
\textbf{Force causes linear acceleration}

What causes angular acceleration?

\pause

\begin{block}{Universal Law: Torque}
$$\boxed{\tau = rF\sin\theta}$$
Torque equals lever arm times force times sine of angle
\end{block}
...
\textbf{Direction:} Same as the angular acceleration it produces
...
\textbf{To maximize torque:}
\begin{itemize}
\item Apply force far from pivot (large $r$) \pause
\item Apply force perpendicular to lever arm ($\theta = 90^\circ$, so $\sin\theta = 1$) \pause
\item Apply larger force (large $F$)
\end{itemize}
...
\textbf{Given:}
\begin{itemize}
\item Initial $\omega_0 = 220$ rad/s
\item Final $\omega = 0$ (stops)
\item $\alpha = -300$ rad/s$^2$
\end{itemize}

\textbf{Find:} Time $t$ for reel to stop
...
\textbf{Equation:} $\omega = \omega_0 + \alpha t$
...
\textbf{Substitute:}
$$t = \frac{0 - 220 \text{ rad/s}}{-300 \text{ rad/s}^2} = \frac{-220}{-300}$$
...
\begin{itemize}
\item Force $F = 250$ N
\item Lever arm $r = 1.50$ m
\item Angle $\theta = 90^\circ$ (perpendicular)
\end{itemize}

\textbf{Find:} Torque $\tau$ produced
...
\textbf{Equation:} $\tau = rF\sin\theta$

\pause

\textbf{Substitute:} $\theta = 90^\circ$, so $\sin 90^\circ = 1$

\pause

$$\tau = (1.50 \text{ m})(250 \text{ N})(1)$$

\pause

$$\boxed{\tau = 375 \text{ N}\cdot\text{m}}$$

\pause

\textbf{Strategy:} Man maximized torque by pushing perpendicular at the outer edge!
\note{[P0] "Self-correct in different color"\\\\
...
\begin{enumerate}
\item Radians are natural units - angles from circle geometry \pause
\item $v = r\omega$ connects spinning to moving \pause
\item Circular motion requires acceleration toward center \pause
\item Centripetal force $F_c = \frac{mv^2}{r}$ keeps objects turning \pause
\item Rotational kinematics mirrors linear kinematics \pause
\item Torque $\tau = rF\sin\theta$ is the rotational force
\end{enumerate}
\end{block}
