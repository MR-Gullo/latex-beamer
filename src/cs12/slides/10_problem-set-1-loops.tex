...
\begin{frame}[fragile]
\frametitle{Key Concept: Extended Integer Types}
\framesubtitle{Long and Long Long for Larger Numbers}

\begin{table}[h]
\centering
\begin{tabular}{|l|c|c|}
\hline
\textbf{Type} & \textbf{Bytes} & \textbf{Range} \\
\hline
\texttt{int} & 4 & $-2.1 \times 10^9$ to $2.1 \times 10^9$ \\
\hline
\texttt{long} & 4 or 8 & Platform dependent \\
\hline
\texttt{long long} & 8 & $-9.2 \times 10^{18}$ to $9.2 \times 10^{18}$ \\
\hline
\texttt{unsigned int} & 4 & $0$ to $4.3 \times 10^9$ \\
\hline
\texttt{unsigned long} & 4 or 8 & Platform dependent \\
\hline
\texttt{unsigned long long} & 8 & $0$ to $1.8 \times 10^{19}$ \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
\item \texttt{int}: Default choice for most applications
\item \texttt{long long}: Large positive numbers like prime factors
\item \texttt{unsigned long long}: Maximum positive range (Project Euler problems)
\end{itemize}

\pause
\begin{minted}[fontsize=\small]{cpp}
// Different approaches for the same large number
signed long long signedNum = 600851475143;      // Can be negative
unsigned long long unsignedNum = 600851475143LL; // Explicitly positive
\end{minted}
\end{frame}
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth]{../images/fib_sequence.jpg}
\caption{Fibonacci sequence calculation showing how each term is the sum of the previous two}
\end{figure}
\end{frame}
