...
\definecolor{ds9grey}{HTML}{666666}
...
\begin{itemize}
\item Understand five different sorting algorithms \pause
\item Be able to implement each sorting algorithm in C++ \pause
\item Know the advantages and disadvantages of each method \pause
\item Recognize the time complexity of different algorithms
\end{itemize}
\end{block}
\note{[P0] Today: 5 sorting algorithms, simple to advanced\\\\
Focus on UNDERSTANDING, not memorizing code\\\\
Ask: "Who's sorted cards by hand?" - that's insertion sort}
\end{frame}
...
\end{block}
\note{[P0] Same array for all algorithms - compare behavior\\\\
Ask students to predict: which will need most swaps?}
\end{frame}

\begin{frame}[fragile]{Bubble Sort: The Code}
\begin{block}{Algorithm Description}
\begin{itemize}
\item Compares adjacent elements and swaps if needed \pause
...
[5,3,8,1] $\rightarrow$ [\textbf{3,5},8,1] $\rightarrow$ [3,5,8,1] $\rightarrow$ [3,5,\textbf{1,8}]
\pause
\\[0.2cm]
\textbf{Pass 2:} 8 is in place, work on first 3\\
[3,5,1,8] $\rightarrow$ [3,5,1,8] $\rightarrow$ [3,\textbf{1,5},8]
\pause
\\[0.2cm]
...
\end{block}
\note{[P0] Watch largest element bubble to right each pass\\\\
Count swaps - bubble sort does MANY\\\\
Ask: "Why called bubble?" - big values rise like bubbles}
\end{frame}

\begin{frame}[fragile]{Selection Sort: The Code}
\begin{block}{Algorithm Description}
\begin{itemize}
\item Find minimum in unsorted region \pause
...
\end{lstlisting}
\note{[TRACE] min_idx tracks POSITION, not value\\\\
[THE BUG] Don't confuse arr[min_idx] (value) with min_idx (position)\\\\
[COMPLEXITY] O(n squared) - always scans entire unsorted portion}
\end{frame}

\begin{frame}{Selection Sort: Trace Through}
\begin{exampleblock}{Let's Trace: arr = [5, 3, 8, 1]}
\textbf{i=0:} Find min in [5,3,8,1] $\rightarrow$ min\_idx=3 (value 1)\\
Swap arr[3] with arr[0]: [\textbf{1},3,8,\textbf{5}]
\pause
\\[0.2cm]
\textbf{i=1:} Find min in [3,8,5] $\rightarrow$ min\_idx=1 (value 3)\\
Swap arr[1] with arr[1]: [1,\textbf{3},8,5] (no change)
\pause
\\[0.2cm]
...
[DEBUG] Print min_idx after inner loop to verify}
\end{frame}
...
\end{block}
\note{[P0] Selection does exactly n-1 swaps regardless of input\\\\
Contrast with bubble: fewer swaps but same comparisons\\\\
Good when swaps are expensive (large objects)}
\end{frame}

\begin{frame}[fragile]{Insertion Sort: The Code}
\begin{block}{Algorithm Description}
\begin{itemize}
\item Take element, slide it left until correct position \pause
...
{[}5,\underline{5},8,1] $\rightarrow$ insert key: [\textbf{3},5,8,1]
\pause
\\[0.2cm]
\textbf{i=2:} key=8, compare with 5, 8>5 so stop\\
{[}3,5,\textbf{8},1] (no shifts needed)
\pause
\\[0.2cm]
...
\end{block}
\note{[P0] Best for nearly-sorted data - O(n) in best case!\\\\
"Sorting cards" analogy: pick up one, slide into place\\\\
Only O(n²) algorithm that's adaptive}
\end{frame}

\begin{frame}[fragile]{Merge Sort: The Concept}
\begin{block}{Algorithm Description}
\begin{itemize}
\item Divide and conquer approach \pause
\item Splits array in half recursively \pause
...
\end{block}
\note{[P0] First O(n log n) algorithm - BIG improvement\\\\
Notice: split phase does nothing, merge does all work\\\\
Tradeoff: needs extra memory for temp arrays}
\end{frame}

\begin{frame}[fragile]{Quick Sort: The Concept}
\begin{block}{Algorithm Description}
\begin{itemize}
\item Choose a pivot element \pause
\item Partition: smaller elements left, larger elements right \pause
...
\end{block}
\note{[P0] Most-used sort in practice - C++ sort() uses it\\\\
Opposite of merge: partition does work, merge trivial\\\\
Watch how pivot choice affects performance}
\end{frame}
...
\begin{itemize}
\item Bubble Sort: O(n²) \pause
\item Selection Sort: O(n²) \pause
\item Insertion Sort: O(n²) \pause
\item Quick Sort: O(n log n) average, O(n²) worst case \pause
\item Merge Sort: O(n log n)
\end{itemize}
\end{block}
\note{[P0] O(n²) vs O(n log n): 1000 elements = 1M vs 10K operations\\\\
Why quick over merge? Cache-friendly, in-place\\\\
Ask: "When would you choose insertion sort?" - small/nearly sorted}
\end{frame}

\begin{frame}{Summary}
\begin{block}{Key Points}
\begin{itemize}
\item Bubble Sort: Simple but inefficient \pause
\item Selection Sort: Simple and performs well on small lists \pause
\item Insertion Sort: Efficient for small data sets \pause
\item Quick Sort: Generally the fastest in practice \pause
\item Merge Sort: Consistent performance but requires extra space
\end{itemize}
\end{block}
\note{[P0] Real world: sort() uses introsort (quick+heap+insertion)\\\\
Interview prep: know bubble/selection/insertion by heart\\\\
Next class: implement one algorithm from scratch}
\end{frame}
