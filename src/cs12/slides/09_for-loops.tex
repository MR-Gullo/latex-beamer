...
\begin{itemize}
\item Construct for loops with proper syntax (initialization, condition, update)\pause
\item Explain when to use for loops versus while loops\pause
\item Understand variable scope within loop structures\pause
\item Apply compound assignment operators (+=, -=, *=, /=)\pause
\item Debug common for loop syntax errors\pause
\item Implement nested loops for multi-dimensional iterations\pause
...
\begin{itemize}
\item Repeat something a specific number of times (e.g., 10 times)\pause
\item Move from first element to last element of a list\pause
...
\begin{itemize}
\item Unknown number of repetitions\pause
\item Getting user input\pause
\item Problems involving division or multiplication\pause
...
\begin{itemize}
\item The key difference is syntax organization: for loops package initialization, condition, and update in one line.\pause
\item Next slide shows the visual flow that applies to both loop types.\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Loop Flow Diagram}
\includegraphics[width=0.6\textwidth]{../images/While-Loop-Flowchart.jpg}\pause
...
\begin{itemize}
\item They only exist within the loop structure itself.\pause
\item Next slide illustrates what happens when you try to access a loop variable outside its scope.\end{itemize}
\end{frame}
...
\begin{itemize}
\item Think of scope as a boundary: variables created inside cannot escape outside.\pause
\item Next slide shows a visual representation of this concept.\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Scope Visualization}
\includegraphics[width=0.6\textwidth]{../images/scope-loop.png}\pause
...
\begin{itemize}
\item Each iteration of the outer loop runs the entire inner loop.\pause
\item This is useful for working with multi-dimensional data like grids, tables, or coordinate systems.\pause
\item Next slides show examples with different nesting depths.\end{itemize}
\end{frame}
...
\begin{itemize}
\item The outer loop controls rows, inner loop controls columns.\pause
\item Next slide visualizes this concept.\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Nested Loop Visualization}
\includegraphics[width=\textwidth]{../images/Nested-Loop.png}\pause
...
\begin{itemize}
\item Algorithm repeatedly divides by smallest possible divisor.\pause
\item Because we test divisors in order (2, 3, 4, ...), any divisor that works must be prime. Why?\pause
\item If it had smaller factors, we would have already divided them out.\pause
\item Next slide shows conceptual breakdown with smaller number.\end{itemize}
\end{frame}
...
\textbf{Key takeaways:}
\begin{itemize}
\item For loops package initialization, condition, update in one line\pause
\item Best for known iteration counts\pause
\item Loop variables have limited scope (exist only in loop)\pause
\item Compound operators (+=, -=, *=, /=) save typing\pause
\item Use semicolons, not commas, in for loop header\pause
\item Nested loops multiply iteration counts\pause
