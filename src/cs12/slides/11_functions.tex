...
By the end of this lesson, you will be able to:\pause
\begin{itemize}
    \item Identify the components of a function (return type, name, parameters, body)\pause
    \item Write functions with various return types (int, bool, char, void)\pause
    \item Implement functions that accept multiple parameters\pause
    \item Use return statements correctly to send values back to the caller\pause
...
\textbf{Benefits:}\pause
\begin{itemize}
    \item \textbf{Reusability}: Write once, use multiple times\pause
    \item \textbf{Modularity}: Break complex problems into smaller pieces\pause
    \item \textbf{Readability}: Makes code easier to understand\pause
...
\begin{enumerate}
    \item \textbf{Return Type}: The type of data the function returns (int, bool, char, void, etc.)\pause
    \item \textbf{Function Name}: How you reference the function (use descriptive names)\pause
    \item \textbf{Parameters}: Input values the function receives (in parentheses)\pause
...
\textbf{Breaking it down:}\pause
\begin{itemize}
    \item \textbf{Return type}: bool (returns true or false)\pause
    \item \textbf{Name}: isEven (descriptive name)\pause
    \item \textbf{Parameter}: int a (input value)\pause
...
\textbf{What happens:}\pause
\begin{itemize}
    \item The value 22 is passed to parameter \texttt{a}\pause
    \item Function executes: \texttt{22 \% 2 == 0} is true\pause
...
\begin{itemize}
    \item \textbf{int}: Returns integer values\pause
    \item \textbf{bool}: Returns true or false\pause
    \item \textbf{char}: Returns a single character\pause
    \item \textbf{float/double}: Returns decimal numbers\pause
...
\textbf{Parameters:}\pause
\begin{itemize}
    \item Variables listed in the function definition\pause
    \item Placeholders for values the function will receive\pause
    \item Example: \texttt{int a} in \texttt{bool isEven(int a)}
\end{itemize}
\pause

\textbf{Arguments:}\pause
\begin{itemize}
    \item Actual values passed when calling the function\pause
...
\textbf{Key concepts:}\pause
\begin{itemize}
    \item Two parameters of same type\pause
    \item Conditional logic determines return value\pause
...
\textbf{Algorithm:}\pause
\begin{itemize}
    \item Divide by 10 repeatedly to remove digits\pause
    \item Count iterations until no digits remain\pause
...
\textbf{void functions:}\pause
\begin{itemize}
    \item Perform actions without returning values\pause
    \item Use \texttt{return;} (no value) or omit return\pause
...
\textbf{Template with TODOs:}\pause
...
\textbf{Expected behavior:}\pause
\begin{itemize}
    \item \texttt{myAbsolute(25)} returns 25\pause
...
\textbf{Expected behavior:}\pause
\begin{itemize}
    \item \texttt{myPower(2, 3)} returns 8\pause
    \item \texttt{myPower(-3, 3)} returns -27\pause
...
\textbf{Expected behavior:}\pause
\begin{itemize}
    \item \texttt{sumDigits(33333)} returns 15\pause
...
\textbf{Expected behavior:}\pause
\begin{itemize}
    \item \texttt{revDigits(1234)} returns 4321\pause
...
\textbf{Expected behavior:}\pause
\begin{itemize}
    \item \texttt{changeCase('E')} returns 'e'\pause
    \item \texttt{changeCase('f')} returns 'F'\pause
...
\textbf{Functions enable code reuse and modularity:}\pause
\begin{itemize}
    \item Every function has a return type, name, parameters, and body\pause
    \item Return type must match the value returned\pause
    \item void functions perform actions without returning values\pause
    \item Parameters receive values when function is called\pause
    \item Multiple parameters separated by commas
\end{itemize}
\pause

\textbf{Common patterns:}\pause
\begin{itemize}
    \item Conditional returns (max, myAbsolute)\pause
    \item Loop-based algorithms (numberOfDigits, sumDigits)\pause
    \item Digit manipulation (revDigits, sumDigits)\pause
